...
export async function GET(request: NextRequest) {
  ...
  if (!url) {
    return agentErrorResponse({
      ...
      hint: "Add a SY INC docs path, for example ?url=/docs/react/components/button.",
  ...
  if (!page) {
    return agentErrorResponse({
      code: "PAGE_NOT_FOUND",
      hint: "Use /api/agent/search to find a canonical SY INC documentation URL.",
      message: `No SY INC documentation page was found for ${url}.`,
      status: 404,
    });
  }
