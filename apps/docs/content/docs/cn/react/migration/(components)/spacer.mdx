---
title: Spacer
description: Spacer 从 SY INC v2 到 v3 的迁移指南。
---

<Callout type="warning">
  Spacer 组件已在 SY INC v3 中**移除**。请改为直接使用 Tailwind CSS 的 margin 工具类。
</Callout>
...
**v2：** 来自 `@sy-inc/react` 的 `<Spacer>` 组件
...
2. **导入调整**：移除 `import { Spacer } from "@sy-inc/react"`。
...
1. **移除导入**：从 `@sy-inc/react` 的导入中删除 `Spacer`。
