---
title: Avatar
description: Avatar 从 SY INC v2 到 v3 的迁移指南。
---

<Callout type="info">
  完整的 API 参考、样式指南与高级示例请参阅 [v3 Avatar 文档](/docs/react/components/avatar)。本指南只关注从 SY INC v2 的迁移。
</Callout>
...
| `AvatarGroup` | `AvatarGroup` | 独立组件，支持 `max`、`overlap`、`isGrid` 和 `AvatarGroup.Count` |
...
使用独立的 [AvatarGroup](/docs/react/components/avatar-group)，将复合式 Avatar 作为直接子级，以继承组内的 `size`、`color` 和 `variant`。v2 `isBordered` 改用 `overlap="ring"`，`total` 改为显式的 `AvatarGroup.Count`。自定义计数会替代 `max` 生成的自动计数，且不会被截断；计数内容由服务端总数减去可见头像数量得到。
...
11. **AvatarGroup**：使用独立头像组；v2 `total` 改为 `AvatarGroup.Count`，`isBordered` 改为 `overlap="ring"`。
