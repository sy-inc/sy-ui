...
3. **迁移所有组件后，切换到仅 v3：**
   - 删除 v2 依赖项（`@sy-ui/react`, `@sy-ui/theme`)
   - 删除别名
   - 将所有导入更新为`@sy-ui/react`（消除`-v3`后缀）
   - 完成样式迁移
...
1. **安装 v3 软件包** (`@sy-ui/react`, `@sy-ui/styles`)
2. **为尚未迁移的组件安装 v2 组件包**
3. **一次迁移一个组件：**
   - 从依赖中删除 v2 组件包
   - 更新导入以使用`@sy-ui/react`(v3)
   - 将组件代码更新为 v3 API
   - 测试迁移的组件
   - 对于没有 v3 对位的 v2 组件（**Code**、**Image**、**Navbar**、**Ripple**、**Snippet**、**Spacer**、**User**），请将它们保留在原处，它们在共存期间仍会继续工作。
4. **继续，直到所有组件均已迁移**
...
2. **更新导入**
   - 策略A：改变`@sy-ui/react` → `@sy-ui-v3/react`
   - 策略 B：更改组件包 →`@sy-ui/react`
...
2. **指导冲突解决**
   - 确保 CSS 导入顺序：`tailwindcss`首先，然后`@sy-ui/styles`
...
1. **删除 v2 依赖项：**
   - 消除`@sy-ui/react`和`@sy-ui/theme`（策略A）
   - 全部删除`@sy-ui/*`组件包（策略B）
   - 删除别名（策略 A）
...
2. **更新所有导入：**
   - 改变`@sy-ui-v3/react` → `@sy-ui/react`（策略A）
   - 更改组件包导入 →`@sy-ui/react`（策略B）

3. **更新CSS：**
   - 从配置中删除 v2 Tailwind 插件
   - 仅保留`@import "@sy-ui/styles";`
