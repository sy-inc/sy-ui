---
title: 增量迁移
description: 将 SY UI v2 增量迁移到 v3 同时保持两个版本并行工作的分步指南
---

## 概述

通过共存的增量迁移，您可以逐个组件地从 SY UI v2 迁移到 v3，同时保持项目在整个迁移过程中正常运行。此方法使用特殊的设置策略来允许 v2 和 v3 组件并行工作。
...
- **提供商**：v2 需要`SyUIProvider`，v3 没有 - 您可能需要条件提供者包装
...
- **使用组件包** (`@sy-ui/button`, `@sy-ui/card`等）：使用策略B（组件包）
...
3. 从以下位置导入 v3 组件`@sy-ui-v3/react`:
...
3. **迁移所有组件后，切换到仅 v3：**
   - 删除 v2 依赖项（`@sy-inc/react`, `@sy-ui/theme`)
   - 删除别名
...
2. **更新导入**
   - 策略A：改变`@sy-inc/react` → `@sy-ui-v3/react`
...
1. **删除 v2 依赖项：**
   - 消除`@sy-inc/react`和`@sy-ui/theme`（策略A）
   - 全部删除`@sy-ui/*`组件包（策略B）
   - 删除别名（策略 A）
...
2. **更新所有导入：**
   - 改变`@sy-ui-v3/react` → `@sy-inc/react`（策略A）
