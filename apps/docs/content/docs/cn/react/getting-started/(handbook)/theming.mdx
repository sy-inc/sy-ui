...
SY UI 的主题系统构建于 [Tailwind CSS v4](https://tailwindcss.com/docs/theme) 的主题之上。当你导入 `@sy-ui/styles` 时，它会使用 Tailwind 的内置调色板，将其映射到语义化变量，自动在浅色和深色主题之间切换，并使用 CSS 层和 `@theme` 指令进行组织。
...
> **注意**：目录导入（例如 `@sy-ui/styles/components`）会自动解析为其对应的 `index.css` 文件。使用显式文件路径（例如 `@sy-ui/styles/components/button.css`）来导入单个组件的样式。
...
**自定义溢出区域** — 在自己的元素上使用 `@sy-ui/styles` 提供的 `scrollbar`、`scrollbar-thin`、`scrollbar-default` 或 `scrollbar-none` 工具类。基于类的覆盖方式详见 [样式](/docs/handbook/styling)。
