---
title: 样式
description: 使用 CSS、Tailwind 或 CSS-in-JS 为 SY UI 组件设置样式
icon: brush
links:
  themes: true
  tailwind: theme
...
SY UI 组件提供灵活的样式方案：Tailwind CSS 工具类、配合 [BEM](https://getbem.com/) 类名或数据属性的 CSS、CSS-in-JS 库，以及用于动态样式的渲染属性。

## 基础样式

**使用 className：** 所有 SY UI 组件都接受 `className` 属性：
...
SY UI 的滚动插槽在组件 CSS 中使用 `@apply scrollbar`。对于你自己的溢出容器，可使用 `@sy-inc/styles` 提供的工具类：
...
| 工具类 | 效果 |
|---------|--------|
| `scrollbar` | SY UI 滚动条滑块（读取主题的 `--scrollbar-*` 变量） |
| `scrollbar-thin` | SY UI 主题化纤细滚动条 |
...
SY UI 组件通过数据属性公开其状态，类似于 CSS 伪类：
...
SY UI 使用 [BEM 方法论](https://getbem.com/) 来保持类名命名的一致性：
...
所有组件的类名请查看 [@sy-inc/styles/components](https://github.com/sy-ui/sy-ui/tree/main/packages/styles/components)。
