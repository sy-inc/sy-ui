...
SY UI 的变体函数位于 `@sy-ui/styles` 包中，可以独立于 React 使用。这使得 Vue、Svelte 等其他框架也能使用 SY UI 的设计系统：
...
两种导入方式的工作原理完全相同。在为非 React 框架构建，或希望避免引入 React 依赖时，请使用 `@sy-ui/styles`。
...
如需更多控制和类型安全，可以使用变体函数将 SY UI 样式应用到特定框架的组件或自定义元素上。`@sy-ui/styles`（与框架无关）和 `@sy-ui/react`（重新导出）都提供了变体函数。
...
**可用的变体函数：** 每个组件都从 `@sy-ui/styles` 导出其变体函数（`buttonVariants`、`chipVariants`、`linkVariants`、`spinnerVariants` 等）。使用它们可以在保持类型安全的同时，将 SY UI 的设计系统应用到任何元素上。
...
由于 `@sy-ui/styles` 没有 React 依赖，你可以直接在任何框架中使用它：
```vue
<script setup>
import { buttonVariants } from '@sy-ui/styles';
