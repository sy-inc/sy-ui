...
使用 `variant="cell"` 将开关渲染为整行的设置项，整行都是点击区域。
在 `default` 背景上请使用 `variant="cell-secondary"`。
...
Switch 组件使用以下 CSS 类（[查看源码样式](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/components/switch.css)）：
...
- `.switch--lg` - 大尺寸变体
- `.switch--cell` - 整行设置项变体
- `.switch--cell-secondary` - 位于 `default` 背景上的设置项

#### SwitchGroup 类 [!toc]

SwitchGroup 组件使用以下 CSS 类（[查看源码样式](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/components/switch-group.css)）：
...
| `variant` | `'default' \| 'cell' \| 'cell-secondary'` | `'default'` | 将开关渲染为整行设置项 |
