...
把内建的 `InputGroup.PasswordToggle` 放进后缀即可。可见状态由 `InputGroup` 托管，消费方无需自己维护：切换开启时，`type="password"` 的 `InputGroup.Input` 会渲染为 `text`。
...
- `[data-slot="input-group-password-toggle"]` – 后缀中内建的密码可见性切换按钮。将 `--button-bg-hover` 与 `--button-bg-pressed` 置为透明，保持无背景反应
...
- **InputGroup.PasswordToggle** - 内建的密码显示/隐藏按钮
...
渲染一个透明的纯图标 [Button](/docs/components/button)（悬停与按下都不改变背景），用于切换该组的密码可见状态。必须放在 `InputGroup.Suffix` 内（后缀会自动去掉内边距、按钮撑满 40px，与纯图标后缀等宽，无需额外 `className`），且其控制的 `InputGroup.Input` 需设置 `type="password"`；其他输入类型不受影响。可见状态为非受控，由 `InputGroup` 托管。

| Prop | 类型 | 默认值 | 描述 |
|------|------|--------|------|
| `showLabel` | `string` | `"Show password"` | 密码隐藏时按钮的无障碍名称 |
| `hideLabel` | `string` | `"Hide password"` | 密码可见时按钮的无障碍名称 |
| `className` | `string` | - | CSS 类 |
