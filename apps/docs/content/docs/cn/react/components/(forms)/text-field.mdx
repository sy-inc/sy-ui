...
| `fullWidth` | `boolean` | `false` | TextField 是否占满容器宽度。会下发到子级的 Input、TextArea 与 InputGroup，无需在子级重复设置。 |
| `variant` | `"primary" \| "secondary"` | `"primary"` | 视觉变体。会下发到子级的 Input、TextArea 与 InputGroup，无需在子级重复设置。 |
| `id` | `string` | - | 元素的唯一 id。 |
...
- **Label** — `@sy-inc/react` 的字段标签组件
- **Input** — `@sy-inc/react` 的单行文本输入
- **TextArea** — `@sy-inc/react` 的多行文本输入
- **Description** — `@sy-inc/react` 的辅助说明组件
- **FieldError** — `@sy-inc/react` 的校验错误信息组件
