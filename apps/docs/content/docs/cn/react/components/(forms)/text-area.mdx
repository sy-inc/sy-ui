...
- **`bare`** — 无边框、无背景、无阴影、无焦点环，供外层 shell 自行绘制字段外观（见 [PromptInput](/docs/components/prompt-input)）
...
`autoGrow` 通过 CSS `field-sizing: content` 让文本域高度跟随内容变化。配合 `rows={1}` 可以从单行开始。不支持 `field-sizing` 的浏览器退化为 `rows` 指定的固定高度。
...
- `.textarea--secondary` – 低强调表面，无阴影
- `.textarea--bare` – 无边框、背景、阴影、圆角与焦点环
- `.textarea--full-width` – 撑满容器宽度
- `.textarea--auto-grow` – 随内容增高
...
| `autoGrow` | `boolean` | `false` | 随内容增高（`field-sizing: content`）；浏览器不支持时退化为 `rows` 高度。 |
| `fullWidth` | `boolean` | `false` | TextArea 是否占满容器宽度 |
| `variant` | `"primary" \| "secondary" \| "bare"` | `"primary"` | 组件的视觉变体。`primary` 为默认带阴影样式。`secondary` 为弱强调、无阴影变体，适合用于 surface 上。`bare` 去掉全部字段外观，交给外层 shell。 |
