...
**迁移步骤：**
1. 单独导入 `SwitchGroup`：`import { Switch, SwitchGroup } from "@sy-inc/react"`
...
- Switch 的迁移完成后，请确认 `SwitchGroup` 已被导入：`import { Switch, SwitchGroup } from "@sy-inc/react"`
...
3. 查阅组件文档中的示例
4. 反馈问题：[GitHub Issues](https://github.com/sy-inc/sy-inc/issues)
