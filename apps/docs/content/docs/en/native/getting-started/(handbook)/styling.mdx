---
title: Styling
description: Style SY UI Native components with Tailwind or StyleSheet API
icon: brush
links:
  themes: true
  tailwind: theme
...
SY UI Native components provide flexible styling options: Tailwind CSS utilities, StyleSheet API, and render props for dynamic styling.

## Styling Principles

SY UI Native is built with `className` as the go-to styling solution. You can use Tailwind CSS classes via the `className` prop on all components.
...
**Using className:** All SY UI Native components accept `className` props:
...
```tsx
import { RadioGroup, Label, cn } from 'sy-ui-native';
...
```tsx
import { Button } from 'sy-ui-native';
import type { ButtonRootProps } from 'sy-ui-native';
...
Each SY UI Native component exports a `classNames` object that contains the same styling functions used internally by the component. This is particularly useful when you want to style your own custom components to match the appearance of SY UI Native components.

For example, you can style a custom `Link` component to look like a `Button`:

```tsx
import { buttonClassNames, cn } from 'sy-ui-native';
...
The `classNames` functions accept the same variant props as the components themselves, allowing you to maintain visual consistency across your custom components and SY UI Native components.


## Responsive Design

SY UI Native supports Tailwind's responsive breakpoint system via [Uniwind](https://docs.uniwind.dev/breakpoints). Use breakpoint prefixes like `sm:`, `md:`, `lg:`, and `xl:` to apply styles conditionally based on screen width.
...
SY UI Native provides utility functions to assist with styling components.
