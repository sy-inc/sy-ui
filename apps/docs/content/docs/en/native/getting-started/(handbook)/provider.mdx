---
title: Provider
description: Configure SY INC Native provider with text, text input, animation, toast, and layout direction settings
icon: chevrons-expand-horizontal
---

The `SyIncNativeProvider` is the root provider component that configures and initializes SY INC Native in your React Native application. It provides global configuration and portal management for your application.

## Overview

The provider serves as the main entry point for SY INC Native, wrapping your application with essential contexts and configurations:
...
- **Text Configuration**: Global text component settings for consistency across all SY INC components
- **Text Input Configuration**: Global text input settings for consistency across all SY INC input components
...
Global settings for all Text components within SY INC Native. These props are carefully selected to include only those that make sense to configure globally across all Text components in the application:

```tsx
import { SyIncNativeProvider } from 'sy-inc-native';
import type { SyIncNativeConfig } from 'sy-inc-native';

const config: SyIncNativeConfig = {
  textProps: {
...
export default function App() {
  return (
    <SyIncNativeProvider config={config}>
      {/* Your app content */}
    </SyIncNativeProvider>
  );
}
...
Global settings for all TextInput-based components within SY INC Native (e.g. Input, TextArea, SearchField, InputGroup, InputOTP). These props are carefully selected to include only those that make sense to configure globally across all inputs in the application:
...
Tell SY INC Native components which layout direction they render in:
...
The `SyIncNativeProvider` internally composes multiple providers:
...
`SyIncNativeProviderRaw` is a lightweight variant of `SyIncNativeProvider` designed for bundle optimization. It excludes `ToastProvider` and `PortalHost`, giving you a bare minimum starting point where you only install and add what you actually need. Its `SyIncNativeConfigRaw` config accepts the same options as `SyIncNativeConfig` except `toast`, so `textProps`, `textInputProps`, `animation`, `devInfo`, and `isRTL` all behave identically.

### When to Use

Use `SyIncNativeProviderRaw` when you want full control over which dependencies are included in your bundle. With the raw provider imported from `sy-inc-native/provider-raw`, the following dependencies are optional and only required if you use the corresponding components:
...
import {
  ...
  type SyIncNativeConfigRaw,
} from 'sy-inc-native/provider-raw';
import { GestureHandlerRootView } from 'react-native-gesture-handler';

const config: SyIncNativeConfigRaw = {
  textProps: {
    maxFontSizeMultiplier: 1.5,
  },
};
...
export default function App() {
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <SyIncNativeProviderRaw config={config}>
        {/* Your app content */}
      </SyIncNativeProviderRaw>
    </GestureHandlerRootView>
  );
...
import { PortalHost } from 'sy-inc-native/portal';
import { ToastProvider } from 'sy-inc-native/toast';
...
Always use a single `SyIncNativeProvider` at the root of your app. Don't nest multiple providers:

```tsx
// ❌ Bad
<SyIncNativeProvider>
  <SomeComponent>
    <SyIncNativeProvider> {/* Don't do this */}
      <AnotherComponent />
    </SyIncNativeProvider>
  </SomeComponent>
</SyIncNativeProvider>
...
function App() {
  return (
    <SyIncNativeProvider config={config}>{/* ... */}</SyIncNativeProvider>
  );
}
```
...
```tsx
import { SyIncNativeProvider, type SyIncNativeConfig } from 'sy-inc-native';
