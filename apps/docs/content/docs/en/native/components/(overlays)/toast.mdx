...
<NativeVideoPlayerView
  target="auto"
  srcLight="https://assets.sy-inc.com/docs/native/components/videos/toast-docs-light.mp4"
  srcDark="https://assets.sy-inc.com/docs/native/components/videos/toast-docs-dark.mp4"
/>

## Import

```tsx
import { Toast, useToast } from 'sy-inc-native';
```
...
```tsx
import { Button, Toast, useToast, useThemeColor } from 'sy-inc-native';
...
You can find more examples in the [GitHub repository](<https://github.com/sy-inc/sy-inc-native/blob/main/example/src/app/(home)/components/toast.tsx>).

## Global Configuration

Configure toast behavior globally using `SyIncNativeProvider` config prop. Global configs serve as defaults for all toasts unless overridden locally.
...
Props for configuring toast behavior globally via `SyIncNativeProvider` config prop.
...
Hook to access toast functionality. Must be used within a `ToastProvider` (provided by `SyIncNativeProvider`).
...
Toast uses FullWindowOverlay on iOS. To enable the React Native element inspector during development, set `disableFullWindowOverlay={true}` on `ToastProvider` (via `config.toast` when using SyIncNativeProvider). Tradeoff: toasts will not appear above native modals when disabled.
