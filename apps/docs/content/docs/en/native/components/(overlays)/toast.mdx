...
<NativeVideoPlayerView
  target="auto"
  srcLight="https://assets.sy-ui.com/docs/native/components/videos/toast-docs-light.mp4"
  srcDark="https://assets.sy-ui.com/docs/native/components/videos/toast-docs-dark.mp4"
/>

## Import

```tsx
import { Toast, useToast } from 'sy-ui-native';
```
...
```tsx
import { Button, Toast, useToast, useThemeColor } from 'sy-ui-native';
...
You can find more examples in the [GitHub repository](<https://github.com/sy-ui/sy-ui-native/blob/main/example/src/app/(home)/components/toast.tsx>).

## Global Configuration

Configure toast behavior globally using `SyUINativeProvider` config prop. Global configs serve as defaults for all toasts unless overridden locally.
...
Props for configuring toast behavior globally via `SyUINativeProvider` config prop.
...
Hook to access toast functionality. Must be used within a `ToastProvider` (provided by `SyUINativeProvider`).
...
Toast uses FullWindowOverlay on iOS. To enable the React Native element inspector during development, set `disableFullWindowOverlay={true}` on `ToastProvider` (via `config.toast` when using SyUINativeProvider). Tradeoff: toasts will not appear above native modals when disabled.
