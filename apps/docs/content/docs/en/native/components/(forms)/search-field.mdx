...
```tsx
import { SearchField } from 'sy-inc-native';
```
...
```tsx
import { Description, Label, SearchField } from 'sy-inc-native';
...
You can find more examples in the [GitHub repository](<https://github.com/sy-inc/sy-inc-native/blob/main/example/src/app/(home)/components/search-field.tsx>).
...
```tsx
import { useSearchField } from 'sy-inc-native';
