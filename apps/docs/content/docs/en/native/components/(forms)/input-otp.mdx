...
```tsx
import { InputOTP } from 'sy-ui-native';
```
...
```tsx
import { InputOTP, REGEXP_ONLY_CHARS } from 'sy-ui-native';
...
```tsx
import { InputOTP, useBottomSheetAwareHandlers } from 'sy-ui-native';
...
```tsx
import { InputOTP, Label, Description, type InputOTPRef } from 'sy-ui-native';
...
You can find more examples in the [GitHub repository](https://github.com/sy-ui/sy-ui-native/blob/main/example/src/app/(home)/components/input-otp.tsx).
