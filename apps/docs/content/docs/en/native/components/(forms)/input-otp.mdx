...
```tsx
import { InputOTP } from 'sy-inc-native';
```
...
```tsx
import { InputOTP, REGEXP_ONLY_CHARS } from 'sy-inc-native';
...
```tsx
import { InputOTP, useBottomSheetAwareHandlers } from 'sy-inc-native';
...
```tsx
import { InputOTP, Label, Description, type InputOTPRef } from 'sy-inc-native';
...
You can find more examples in the [GitHub repository](https://github.com/sy-inc/sy-inc-native/blob/main/example/src/app/(home)/components/input-otp.tsx).
