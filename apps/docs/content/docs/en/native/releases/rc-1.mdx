---
title: RC 1
description: Alert component, standalone Radio component, Select TriggerIndicator, SyUINativeProviderRaw, disableFullWindowOverlay prop, styles prop expansion, theme surface refactor
github:
  pull: 284
---
...
RC 1 marks the first Release Candidate for SY UI Native, signaling that the library is approaching production readiness. This release introduces the Alert compound component with accessibility-first design and status variants, extracts Radio into a standalone component with dual-mode operation, and adds the animated Select.TriggerIndicator subcomponent. It also delivers a lightweight SyUINativeProviderRaw for bundle optimization, a new `disableFullWindowOverlay` prop for iOS debugging, unified `styles` prop support across six components, and a theme refactor that replaces calculated surface colors with explicit theme-defined variables. Several critical bug fixes—including InputOTP in BottomSheet, toast text clipping, and element inspector compatibility—round out the release.
...
<NativeVideoPlayerView
  target="component"
  slug="alert"
  srcLight="https://assets.sy-ui.com/docs/native/components/videos/alert-docs-light.mp4"
  srcDark="https://assets.sy-ui.com/docs/native/components/videos/alert-docs-dark.mp4"
/>
...
```tsx
import { Alert } from "sy-ui-native";
...
**Related PR:** [#284](https://github.com/sy-ui/sy-ui-native/pull/284)
...
```tsx
import { Select } from "sy-ui-native";
...
**Related PR:** [#274](https://github.com/sy-ui/sy-ui-native/pull/274)
...
**Related PR:** [#229](https://github.com/sy-ui/sy-ui-native/pull/229)
...
**Related PR:** [#277](https://github.com/sy-ui/sy-ui-native/pull/277)
...
**Related PR:** [#281](https://github.com/sy-ui/sy-ui-native/pull/281)
...
```tsx
import { Accordion, Label } from "sy-ui-native";
...
**Related PR:** [#271](https://github.com/sy-ui/sy-ui-native/pull/271)
...
```tsx
import { Dialog } from "sy-ui-native";
...
For Toast, the prop is passed via `config.toast` when using `SyUINativeProvider`.

**Related PR:** [#283](https://github.com/sy-ui/sy-ui-native/pull/283)

### SyUINativeProviderRaw

A new lightweight provider variant, `SyUINativeProviderRaw`, excludes `ToastProvider` and `PortalHost` to give consumers full control over which dependencies are bundled.
...
**Related PR:** [#285](https://github.com/sy-ui/sy-ui-native/pull/285)
...
```tsx
import { Button, Select } from "sy-ui-native";
...
```tsx
import { ControlField } from "sy-ui-native";
...
**Related PR:** [#286](https://github.com/sy-ui/sy-ui-native/pull/286)
...
```tsx
// Before
import { RadioGroup } from "sy-ui-native";
...
// After
import { Radio, RadioGroup } from "sy-ui-native";
...
- **[Issue #229](https://github.com/sy-ui/sy-ui-native/issues/229)**: Fixed InputOTP not working inside BottomSheet. The InputOTP component now functions correctly when rendered within a BottomSheet overlay, resolving input focus and interaction issues that previously prevented OTP entry in bottom sheet contexts.

- **[Issue #265](https://github.com/sy-ui/sy-ui-native/issues/265)**: Fixed the first letter of Toast description text being clipped. The toast styling refactor replaces the border-based padding approach with proper padding and placeholder views, ensuring all text content is fully visible regardless of toast stacking configuration.

- **[Issue #272](https://github.com/sy-ui/sy-ui-native/issues/272)**: Fixed FullWindowOverlay blocking the React Native element inspector on iOS. The new `disableFullWindowOverlay` prop on portal components allows developers to render overlay content in the main window during development, restoring element inspector functionality.

**Related PRs:**

- [#229](https://github.com/sy-ui/sy-ui-native/pull/229)
- [#283](https://github.com/sy-ui/sy-ui-native/pull/283)
...
- [Provider](/docs/native/getting-started/provider) - SyUINativeProviderRaw documentation and provider hierarchy
