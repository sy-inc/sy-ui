---
title: RC 1
description: Alert component, standalone Radio component, Select TriggerIndicator, SyIncNativeProviderRaw, disableFullWindowOverlay prop, styles prop expansion, theme surface refactor
github:
  pull: 284
---
...
RC 1 marks the first Release Candidate for SY INC Native, signaling that the library is approaching production readiness. This release introduces the Alert compound component with accessibility-first design and status variants, extracts Radio into a standalone component with dual-mode operation, and adds the animated Select.TriggerIndicator subcomponent. It also delivers a lightweight SyIncNativeProviderRaw for bundle optimization, a new `disableFullWindowOverlay` prop for iOS debugging, unified `styles` prop support across six components, and a theme refactor that replaces calculated surface colors with explicit theme-defined variables. Several critical bug fixes—including InputOTP in BottomSheet, toast text clipping, and element inspector compatibility—round out the release.
...
<NativeVideoPlayerView
  target="component"
  slug="alert"
  srcLight="https://assets.sy-inc.com/docs/native/components/videos/alert-docs-light.mp4"
  srcDark="https://assets.sy-inc.com/docs/native/components/videos/alert-docs-dark.mp4"
/>
...
```tsx
import { Alert } from "sy-inc-native";
...
**Related PR:** [#284](https://github.com/sy-inc/sy-inc-native/pull/284)
...
```tsx
import { Select } from "sy-inc-native";
...
**Related PR:** [#274](https://github.com/sy-inc/sy-inc-native/pull/274)
...
**Related PR:** [#229](https://github.com/sy-inc/sy-inc-native/pull/229)
...
**Related PR:** [#277](https://github.com/sy-inc/sy-inc-native/pull/277)
...
**Related PR:** [#281](https://github.com/sy-inc/sy-inc-native/pull/281)
...
```tsx
import { Accordion, Label } from "sy-inc-native";
...
**Related PR:** [#271](https://github.com/sy-inc/sy-inc-native/pull/271)
...
```tsx
import { Dialog } from "sy-inc-native";
...
For Toast, the prop is passed via `config.toast` when using `SyIncNativeProvider`.

**Related PR:** [#283](https://github.com/sy-inc/sy-inc-native/pull/283)

### SyIncNativeProviderRaw

A new lightweight provider variant, `SyIncNativeProviderRaw`, excludes `ToastProvider` and `PortalHost` to give consumers full control over which dependencies are bundled.

**New Capability:**

```tsx
import { SyIncNativeProviderRaw } from "sy-inc-native/provider-raw";
...
**Related PR:** [#285](https://github.com/sy-inc/sy-inc-native/pull/285)
...
```tsx
import { Button, Select } from "sy-inc-native";
...
```tsx
import { ControlField } from "sy-inc-native";
...
**Related PR:** [#286](https://github.com/sy-inc/sy-inc-native/pull/286)
...
```tsx
// Before
import { RadioGroup } from "sy-inc-native";
...
// After
import { Radio, RadioGroup } from "sy-inc-native";
...
- **[Issue #229](https://github.com/sy-inc/sy-inc-native/issues/229)**: Fixed InputOTP not working inside BottomSheet. The InputOTP component now functions correctly when rendered within a BottomSheet overlay, resolving input focus and interaction issues that previously prevented OTP entry in bottom sheet contexts.

- **[Issue #265](https://github.com/sy-inc/sy-inc-native/issues/265)**: Fixed the first letter of Toast description text being clipped. The toast styling refactor replaces the border-based padding approach with proper padding and placeholder views, ensuring all text content is fully visible regardless of toast stacking configuration.

- **[Issue #272](https://github.com/sy-inc/sy-inc-native/issues/272)**: Fixed FullWindowOverlay blocking the React Native element inspector on iOS. The new `disableFullWindowOverlay` prop on portal components allows developers to render overlay content in the main window during development, restoring element inspector functionality.

**Related PRs:**

- [#229](https://github.com/sy-inc/sy-inc-native/pull/229)
- [#283](https://github.com/sy-inc/sy-inc-native/pull/283)
...
- [Provider](/docs/native/getting-started/provider) - SyIncNativeProviderRaw documentation and provider hierarchy
