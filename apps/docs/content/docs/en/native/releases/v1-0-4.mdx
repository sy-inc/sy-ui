...
```tsx
import { Typography } from "sy-inc-native";
...
export function Example() {
  return (
    <View className="flex-1 justify-center px-5 gap-4">
      ...
      <Typography.Code>npm install sy-inc-native</Typography.Code>
    </View>
  );
}
```
...
<Callout type="info">
  **No migration required.** Existing `import { Text } from "sy-inc-native"` continues to work via deprecated re-exports. Migrate to `Typography` at your convenience before a future major release removes the deprecated aliases.
</Callout>

**Related PR:** [#417](https://github.com/sy-inc/sy-inc-native/pull/417)

### Vibrant Theme Palette (Optional)

A new optional `sy-inc-native/styles/vibrant` stylesheet is available for apps that prefer more saturated brand colors on soft variants. It keeps the readability improvements from the refined soft-foreground tokens while preserving high color saturation for icons and text on soft backgrounds.
...
import "sy-inc-native/styles";
import "sy-inc-native/styles/vibrant";
```

Import `sy-inc-native/styles/vibrant` after the base styles to override soft-foreground tokens with the more saturated values. No component code changes are required to opt in.

**Related PR:** [#420](https://github.com/sy-inc/sy-inc-native/pull/420)
...
- A new optional `sy-inc-native/styles/vibrant` export preserves saturated brand colors on soft variants for apps that want a more vivid look
...
**Related PR:** [#421](https://github.com/sy-inc/sy-inc-native/pull/421)
...
The peer dependency range for `@gorhom/bottom-sheet` has been bumped from `^5.2.8` to `^5.2.9`. Apps that use SY INC Native's [BottomSheet](/docs/native/components/bottom-sheet) component (or any of `Menu` / `Popover` / `Select` with `presentation="bottom-sheet"`) should ensure they are on a compatible version:
...
- Example app slug/bundle renamed to `sy-inc-native-oss`; Android `WithStateToggle` padding adjusted
...
import { Popover } from "sy-inc-native";
...
import { Text, textClassNames, type TextProps } from "sy-inc-native";
