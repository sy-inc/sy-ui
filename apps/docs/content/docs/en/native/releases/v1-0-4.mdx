...
```tsx
import { Typography } from "sy-ui-native";
...
export function Example() {
  return (
    <View className="flex-1 justify-center px-5 gap-4">
      ...
      <Typography.Code>npm install sy-ui-native</Typography.Code>
    </View>
  );
}
```
...
<Callout type="info">
  **No migration required.** Existing `import { Text } from "sy-ui-native"` continues to work via deprecated re-exports. Migrate to `Typography` at your convenience before a future major release removes the deprecated aliases.
</Callout>

**Related PR:** [#417](https://github.com/sy-ui/sy-ui-native/pull/417)

### Vibrant Theme Palette (Optional)

A new optional `sy-ui-native/styles/vibrant` stylesheet is available for apps that prefer more saturated brand colors on soft variants. It keeps the readability improvements from the refined soft-foreground tokens while preserving high color saturation for icons and text on soft backgrounds.
...
import "sy-ui-native/styles";
import "sy-ui-native/styles/vibrant";
```

Import `sy-ui-native/styles/vibrant` after the base styles to override soft-foreground tokens with the more saturated values. No component code changes are required to opt in.

**Related PR:** [#420](https://github.com/sy-ui/sy-ui-native/pull/420)
...
- A new optional `sy-ui-native/styles/vibrant` export preserves saturated brand colors on soft variants for apps that want a more vivid look
...
**Related PR:** [#421](https://github.com/sy-ui/sy-ui-native/pull/421)
...
The peer dependency range for `@gorhom/bottom-sheet` has been bumped from `^5.2.8` to `^5.2.9`. Apps that use SY UI Native's [BottomSheet](/docs/native/components/bottom-sheet) component (or any of `Menu` / `Popover` / `Select` with `presentation="bottom-sheet"`) should ensure they are on a compatible version:
...
- Example app slug/bundle renamed to `sy-ui-native-oss`; Android `WithStateToggle` padding adjusted
...
import { Popover } from "sy-ui-native";
...
import { Text, textClassNames, type TextProps } from "sy-ui-native";
