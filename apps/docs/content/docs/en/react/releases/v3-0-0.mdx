...
**Styles separated from implementation:** `@sy-ui/styles` is standalone CSS. `@sy-ui/react` handles behavior. Use the styles with React, plain HTML + Tailwind, or any framework. BEM class names make every slot customizable globally. Swap themes to change not just variables, but how components look and feel.

**Headless when you want it:** Remove the `@sy-ui/styles` import and you have a headless library. We maintain functionality and accessibility. You focus on your product.
...
```css
@import "tailwindcss";
@import "@sy-ui/styles/base" layer(base);
@import "@sy-ui/styles/themes/default" layer(theme);
@import "@sy-ui/styles/components/button.css" layer(components);
@import "@sy-ui/styles/components/card.css" layer(components);
```
...
// Subpath import
import { Button } from "@sy-ui/react/button";
import { Card } from "@sy-ui/react/card";
import { Table } from "@sy-ui/react/table";
```
...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  ...
  <Tab value="pnpm">
    ```bash
    pnpm add @sy-ui/styles @sy-ui/react
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @sy-ui/styles @sy-ui/react
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @sy-ui/styles @sy-ui/react
    ```
  </Tab>
</Tabs>
