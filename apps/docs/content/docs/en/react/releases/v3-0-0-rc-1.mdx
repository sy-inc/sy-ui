...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
    ```bash
    npm i @sy-ui/styles@rc @sy-ui/react@rc
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm add @sy-ui/styles@rc @sy-ui/react@rc
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @sy-ui/styles@rc @sy-ui/react@rc
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @sy-ui/styles@rc @sy-ui/react@rc
    ```
  </Tab>
</Tabs>
...
Table and ListBox now support virtualization via React Aria's `Virtualizer` for large datasets. `Virtualizer`, `TableLayout`, and `ListLayout` are re-exported from `@sy-ui/react`.
...
`@sy-ui/react` now supports per-component subpath entrypoints for more explicit imports ([#6301](https://github.com/sy-ui/sy-ui/pull/6301)):
