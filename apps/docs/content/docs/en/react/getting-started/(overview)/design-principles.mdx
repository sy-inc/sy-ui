...
```tsx
import { Tabs, TabList, Tab, TabPanel } from '@sy-ui/react';
...
import { Alert, AlertIcon, CardHeader, AccordionTrigger } from '@sy-ui/react';
...
Styles (`@sy-ui/styles`) are separate from logic (`@sy-ui/react`), enabling use with any framework or vanilla HTML. See [Tailwind Play example](https://play.tailwindcss.com/vMYXzKPyUx).
