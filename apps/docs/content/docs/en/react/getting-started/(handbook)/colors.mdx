---
title: Colors
description: Color palette and theming system for SY INC v3
icon: bucket-paint
links:
  themes: true
  tailwind: theme
...
SY INC's color system is built around semantic intent, not visual abundance. Instead of exposing large raw palettes, the system defines a small, meaningful set of color roles that cover the majority of interface needs.
...
The complete theme definition can be found in ([variables.css](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/themes/default/variables.css)). This theme automatically switches between light and dark modes based on the `class="dark"` or `data-theme="dark"` attributes.

<CollapsibleCode lang="css" code={`  @layer base {
    /* SY INC Default Theme */
    :root {
      color-scheme: light;
