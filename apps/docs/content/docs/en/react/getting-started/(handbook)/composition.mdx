...
SY INC uses composition patterns to create flexible, customizable components. Change the rendered element, compose components together, and maintain full control over markup.

## Framework-Agnostic Styles

SY INC's variant functions are available in the `@sy-inc/styles` package, which can be used independently of React. This enables Vue, Svelte, and other frameworks to use SY INC's design system:

```tsx
// Import directly from @sy-inc/styles (framework-agnostic)
import { buttonVariants } from '@sy-inc/styles';

// Or import from @sy-inc/react (re-exports the same functions)
import { buttonVariants } from '@sy-inc/react';
```

Both imports work identically. Use `@sy-inc/styles` when building for non-React frameworks or when you want to avoid pulling in React dependencies.

## Polymorphic Styling

Apply SY INC styles to any element using variant functions or BEM classes. Extend component styles to framework components, native HTML elements, or custom components with full type safety.
...
When using a custom root element instead of SY INC's Root component, child components cannot access context slots. You can manually pass `className` to child components using variant functions or BEM classes:
...
This approach works because SY INC's variant functions and BEM classes can be applied to any element, giving you complete flexibility to style framework components, native elements, or custom components with SY INC's design system.

## Direct Class Application

The simplest way to style links or other elements is to apply SY INC's [BEM](https://getbem.com/) classes directly. This approach is straightforward and works with any framework or vanilla HTML.
...
This approach works because SY INC uses [BEM](https://getbem.com/) classes that can be applied to any element. It's perfect when you don't need the component's interactive features (like `onPress` handlers) and just want the visual styling.

## Using Variant Functions

For more control and type safety, use variant functions to apply SY INC styles to framework-specific components or custom elements. Variant functions are available from both `@sy-inc/styles` (framework-agnostic) and `@sy-inc/react` (re-exports).
...
**Available variant functions:** Each component exports its variant function (`buttonVariants`, `chipVariants`, `linkVariants`, `spinnerVariants`, and more) from `@sy-inc/styles`. Use them to apply SY INC's design system to any element while maintaining type safety.

## Compound Components

SY INC components are built as compound components—they export multiple parts that work together. Use them in three flexible ways:

**Option 1: Compound pattern (recommended)** — Use the main component directly without `.Root` suffix:

```tsx
import { Alert } from '@sy-inc/react';
...
```tsx
import { Alert, AlertTitle, AlertDescription } from '@sy-inc/react';
...
// Or named export
import { ButtonRoot } from '@sy-inc/react';
...
```tsx
import { Link } from '@sy-inc/react';
import { linkVariants, buttonVariants } from '@sy-inc/styles';
...
Create your own components by composing SY INC primitives:
...
```tsx
import type { ButtonRootProps } from "@sy-inc/react";
import type { VariantProps } from "tailwind-variants";

import { Button } from "@sy-inc/react";
import { buttonVariants, tv } from "@sy-inc/styles";
...
```tsx
import type { ButtonRootProps, AvatarRootProps } from "@sy-inc/react";
...
```tsx
import { Button, Avatar } from "@sy-inc/react";
...
Since `@sy-inc/styles` has no React dependencies, you can use it directly in any framework:
```vue
<script setup>
import { buttonVariants } from '@sy-inc/styles';
