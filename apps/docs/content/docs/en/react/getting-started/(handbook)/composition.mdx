...
SY UI's variant functions are available in the `@sy-inc/styles` package, which can be used independently of React. This enables Vue, Svelte, and other frameworks to use SY UI's design system:
...
For more control and type safety, use variant functions to apply SY UI styles to framework-specific components or custom elements. Variant functions are available from both `@sy-inc/styles` (framework-agnostic) and `@sy-inc/react` (re-exports).
...
**Available variant functions:** Each component exports its variant function (`buttonVariants`, `chipVariants`, `linkVariants`, `spinnerVariants`, and more) from `@sy-inc/styles`. Use them to apply SY UI's design system to any element while maintaining type safety.
