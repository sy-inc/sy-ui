...
SY UI scroll slots use `@apply scrollbar` in component CSS. For your own overflow containers, use the utilities from `@sy-ui/styles`:
...
```tsx
import { Button as HeroButton, type ButtonProps } from '@sy-ui/react';
import { buttonVariants, tv, type VariantProps } from '@sy-ui/styles';
...
View all component classes in [@sy-ui/styles/components](https://github.com/sy-ui/sy-ui/tree/main/packages/styles/components).
