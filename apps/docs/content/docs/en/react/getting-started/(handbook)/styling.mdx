---
title: Styling
description: Style SY UI components with CSS, Tailwind, or CSS-in-JS
icon: brush
links:
  themes: true
  tailwind: theme
...
SY UI components provide flexible styling options: Tailwind CSS utilities, CSS with [BEM](https://getbem.com/) classes or data attributes, CSS-in-JS libraries, and render props for dynamic styling.

## Basic Styling

**Using className:** All SY UI components accept `className` props:
...
SY UI scroll slots use `@apply scrollbar` in component CSS. For your own overflow containers, use the utilities from `@sy-inc/styles`:
...
| Utility | Effect |
|---------|--------|
| `scrollbar` | SY UI thumb (reads theme `--scrollbar-*` variables) |
| `scrollbar-thin` | SY UI themed thin scrollbar |
...
SY UI components expose their state through data attributes, similar to CSS pseudo-classes:
...
SY UI uses [BEM methodology](https://getbem.com/) for consistent class naming:
...
View all component classes in [@sy-inc/styles/components](https://github.com/sy-ui/sy-ui/tree/main/packages/styles/components).
