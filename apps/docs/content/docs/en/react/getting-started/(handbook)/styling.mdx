---
title: Styling
description: Style SY INC components with CSS, Tailwind, or CSS-in-JS
icon: brush
links:
  themes: true
  tailwind: theme
...
SY INC components provide flexible styling options: Tailwind CSS utilities, CSS with [BEM](https://getbem.com/) classes or data attributes, CSS-in-JS libraries, and render props for dynamic styling.

## Basic Styling

**Using className:** All SY INC components accept `className` props:
...
SY INC scroll slots use `@apply scrollbar` in component CSS. For your own overflow containers, use the utilities from `@sy-inc/styles`:
...
| Utility | Effect |
|---------|--------|
| `scrollbar` | SY INC thumb (reads theme `--scrollbar-*` variables) |
| `scrollbar-thin` | SY INC themed thin scrollbar |
...
SY INC components expose their state through data attributes, similar to CSS pseudo-classes:
...
SY INC uses [BEM methodology](https://getbem.com/) for consistent class naming:
...
```tsx
import { Button as HeroButton, type ButtonProps } from '@sy-inc/react';
import { buttonVariants, tv, type VariantProps } from '@sy-inc/styles';
...
View all component classes in [@sy-inc/styles/components](https://github.com/sy-inc/sy-inc/tree/main/packages/styles/components).
