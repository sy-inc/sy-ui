---
title: Theming
description: Customize SY UI's design system with CSS variables and global styles
icon: palette
links:
  themes: true
  tailwind: theme
...
SY UI uses CSS variables and [BEM](https://getbem.com/) classes for theming. Customize everything from colors to component styles using standard CSS.
...
> **Dark mode**: For a complete setup guide with `next-themes` and SY UI's `useTheme` hook, see [Dark Mode](/docs/handbook/dark-mode).
...
SY UI defines three types of variables in [`variables.css`](https://github.com/sy-ui/sy-ui/blob/v3/packages/styles/themes/default/variables.css):
...
For a complete reference, see: [Colors Documentation](/docs/handbook/colors), [Default Theme Variables](https://github.com/sy-ui/sy-ui/blob/v3/packages/styles/themes/default/variables.css), [Shared Theme Utilities](https://github.com/sy-ui/sy-ui/blob/v3/packages/styles/themes/shared/theme.css)

**Tailwind theme bridge (`@theme inline`):**

[`themes/shared/theme.css`](https://github.com/sy-ui/sy-ui/blob/v3/packages/styles/themes/shared/theme.css) maps semantic variables to Tailwind tokens (`--color-*`, `--radius-*`, `--ease-*`). Calculated colors reference underlying vars (e.g. `--surface-hover`, `--accent-soft`) from `variables.css` — they are not inlined with `color-mix()` in this file:
...
SY UI applies a shared scrollbar style to component scroll areas (tables, popovers, drawers, and similar). Scrollbars use standard CSS properties (`scrollbar-width`, `scrollbar-color`, `scrollbar-gutter`) — no `::-webkit-scrollbar` overrides.

**Modes** — set `data-scrollbar` on `<html>`, a component root, or a scroll slot:

| Mode | `data-scrollbar` | Behavior |
|------|------------------|----------|
| SY UI thin | *(unset)* or `thin` | Thin thumb from theme tokens |
...
<!-- Restore SY UI scrollbars in one subtree -->
...
**Theme variables** — defined in light and dark theme blocks in [`variables.css`](https://github.com/sy-ui/sy-ui/blob/v3/packages/styles/themes/default/variables.css):
...
> **Note**: Some components hide scrollbars by default (date picker popovers, color picker, secondary tabs). Nested scroll slots (e.g. the calendar year picker inside a date picker) keep SY UI scrollbars because `scrollbar-none` only affects the element it is applied to, not descendants using `@apply scrollbar`.

## Vibrant Palette

By default, SY UI uses accessible soft foreground colors that mix the semantic color with the foreground for better contrast. If you prefer more saturated, vibrant soft foreground colors, add the `data-vibrant-palette` attribute to your root element:
