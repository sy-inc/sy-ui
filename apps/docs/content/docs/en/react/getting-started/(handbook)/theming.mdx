---
title: Theming
description: Customize SY INC's design system with CSS variables and global styles
icon: palette
links:
  themes: true
  tailwind: theme
...
SY INC uses CSS variables and [BEM](https://getbem.com/) classes for theming. Customize everything from colors to component styles using standard CSS.
...
SY INC's theming system is built on top of [Tailwind CSS v4](https://tailwindcss.com/docs/theme)'s theme. When you import `@sy-inc/styles`, it uses Tailwind's built-in color palettes, maps them to semantic variables, automatically switches between light and dark themes, and uses CSS layers and the `@theme` directive for organization.
...
> **Dark mode**: For a complete setup guide with `next-themes` and SY INC's `useTheme` hook, see [Dark Mode](/docs/handbook/dark-mode).
...
@import "@sy-inc/styles/base/base.css" layer(base);

/* Theme variables */
@import "@sy-inc/styles/themes/shared/theme.css" layer(theme);
@import "@sy-inc/styles/themes/default" layer(theme);
/* OR specific theme files */
@import "@sy-inc/styles/themes/default/index.css" layer(theme);
@import "@sy-inc/styles/themes/default/variables.css" layer(theme);

/* Components (all components) */
@import "@sy-inc/styles/components" layer(components);
/* OR specific component files */
@import "@sy-inc/styles/components/index.css" layer(components);
@import "@sy-inc/styles/components/button.css" layer(components);
@import "@sy-inc/styles/components/accordion.css" layer(components);

/* Utilities (optional) */
@import "@sy-inc/styles/utilities" layer(utilities);

/* Variants (optional) */
@import "@sy-inc/styles/variants" layer(utilities);
```

> **Note**: Directory imports (e.g., `@sy-inc/styles/components`) automatically resolve to their `index.css` file. Use explicit file paths (e.g., `@sy-inc/styles/components/button.css`) to import individual component styles.

**Headless mode:** Build your own styles from scratch:

```css
@import "tailwindcss";
@import "@sy-inc/styles/base/base.css";
...
SY INC defines three types of variables in [`variables.css`](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/themes/default/variables.css):
...
For a complete reference, see: [Colors Documentation](/docs/handbook/colors), [Default Theme Variables](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/themes/default/variables.css), [Shared Theme Utilities](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/themes/shared/theme.css)

**Tailwind theme bridge (`@theme inline`):**

[`themes/shared/theme.css`](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/themes/shared/theme.css) maps semantic variables to Tailwind tokens (`--color-*`, `--radius-*`, `--ease-*`). Calculated colors reference underlying vars (e.g. `--surface-hover`, `--accent-soft`) from `variables.css` — they are not inlined with `color-mix()` in this file:
...
SY INC applies a shared scrollbar style to component scroll areas (tables, popovers, drawers, and similar). Scrollbars use standard CSS properties (`scrollbar-width`, `scrollbar-color`, `scrollbar-gutter`) — no `::-webkit-scrollbar` overrides.

**Modes** — set `data-scrollbar` on `<html>`, a component root, or a scroll slot:

| Mode | `data-scrollbar` | Behavior |
|------|------------------|----------|
| SY INC thin | *(unset)* or `thin` | Thin thumb from theme tokens |
...
<!-- Restore SY INC scrollbars in one subtree -->
...
**Theme variables** — defined in light and dark theme blocks in [`variables.css`](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/themes/default/variables.css):
...
**Custom overflow areas** — use the `scrollbar`, `scrollbar-thin`, `scrollbar-default`, or `scrollbar-none` utilities from `@sy-inc/styles` on your own elements. See [Styling](/docs/handbook/styling) for class-based overrides.

> **Note**: Some components hide scrollbars by default (date picker popovers, color picker, secondary tabs). Nested scroll slots (e.g. the calendar year picker inside a date picker) keep SY INC scrollbars because `scrollbar-none` only affects the element it is applied to, not descendants using `@apply scrollbar`.

## Vibrant Palette

By default, SY INC uses accessible soft foreground colors that mix the semantic color with the foreground for better contrast. If you prefer more saturated, vibrant soft foreground colors, add the `data-vibrant-palette` attribute to your root element:
