...
SY UI's theming system is built on top of [Tailwind CSS v4](https://tailwindcss.com/docs/theme)'s theme. When you import `@sy-ui/styles`, it uses Tailwind's built-in color palettes, maps them to semantic variables, automatically switches between light and dark themes, and uses CSS layers and the `@theme` directive for organization.
...
@import "@sy-ui/styles/base/base.css" layer(base);

/* Theme variables */
@import "@sy-ui/styles/themes/shared/theme.css" layer(theme);
@import "@sy-ui/styles/themes/default" layer(theme);
/* OR specific theme files */
@import "@sy-ui/styles/themes/default/index.css" layer(theme);
@import "@sy-ui/styles/themes/default/variables.css" layer(theme);

/* Components (all components) */
@import "@sy-ui/styles/components" layer(components);
/* OR specific component files */
@import "@sy-ui/styles/components/index.css" layer(components);
@import "@sy-ui/styles/components/button.css" layer(components);
@import "@sy-ui/styles/components/accordion.css" layer(components);

/* Utilities (optional) */
@import "@sy-ui/styles/utilities" layer(utilities);

/* Variants (optional) */
@import "@sy-ui/styles/variants" layer(utilities);
```

> **Note**: Directory imports (e.g., `@sy-ui/styles/components`) automatically resolve to their `index.css` file. Use explicit file paths (e.g., `@sy-ui/styles/components/button.css`) to import individual component styles.

**Headless mode:** Build your own styles from scratch:

```css
@import "tailwindcss";
@import "@sy-ui/styles/base/base.css";
...
**Custom overflow areas** — use the `scrollbar`, `scrollbar-thin`, `scrollbar-default`, or `scrollbar-none` utilities from `@sy-ui/styles` on your own elements. See [Styling](/docs/handbook/styling) for class-based overrides.
