---
title: Animation
description: Add smooth animations and transitions to SY UI v3 components
icon: copy-transparent
---

SY UI components support multiple animation approaches: built-in CSS transitions, custom CSS animations, and JavaScript libraries like Framer Motion.

## Built-in Animations

SY UI components use data attributes to expose their state for animation:
...
SY UI components work seamlessly with Framer Motion for advanced animations.
...
**Respecting motion preferences:** SY UI automatically respects user motion preferences using Tailwind's `motion-reduce:` utility. All built-in transitions and animations are disabled when users enable "reduce motion" in their system settings.

SY UI extends Tailwind's `motion-reduce:` variant to support both the native `prefers-reduced-motion` media query and the `data-reduce-motion` attribute.

```css
/* SY UI pattern - uses Tailwind's motion-reduce: */
.button {
...
SY UI automatically detects the user's `prefers-reduced-motion: reduce` setting and disables animations accordingly.
