---
title: Animation
description: Add smooth animations and transitions to SY INC v3 components
icon: copy-transparent
---

SY INC components support multiple animation approaches: built-in CSS transitions, custom CSS animations, and JavaScript libraries like Framer Motion.

## Built-in Animations

SY INC components use data attributes to expose their state for animation:
...
SY INC components work seamlessly with Framer Motion for advanced animations.
...
**Respecting motion preferences:** SY INC automatically respects user motion preferences using Tailwind's `motion-reduce:` utility. All built-in transitions and animations are disabled when users enable "reduce motion" in their system settings.

SY INC extends Tailwind's `motion-reduce:` variant to support both the native `prefers-reduced-motion` media query and the `data-reduce-motion` attribute.

```css
/* SY INC pattern - uses Tailwind's motion-reduce: */
.button {
...
SY INC automatically detects the user's `prefers-reduced-motion: reduce` setting and disables animations accordingly.
