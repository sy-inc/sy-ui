...
1. **Identify hook usage**: Search your codebase for imports from `@sy-ui/react` that include hook names (`useSwitch`, `useInput`, `useCheckbox`, `useRadio`, etc.)
...
```tsx
import { useSwitch } from "@sy-ui/react";
...
**Example:**
```tsx
import { Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Button, useDisclosure } from "@sy-ui/react";
...
**Example:**
```tsx
import { Modal, Button, useOverlayState } from "@sy-ui/react";
