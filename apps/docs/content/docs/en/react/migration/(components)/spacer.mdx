...
**v2:** `<Spacer>` component from `@sy-ui/react`  
...
<Tabs items={["v2", "v3"]}>
  <Tab value="v2">
    ```tsx
    import { Spacer } from "@sy-ui/react";
...
<Tabs items={["v2", "v3"]}>
  <Tab value="v2">
    ```tsx
    import { Spacer, Button } from "@sy-ui/react";
...
2. **Import Change**: Remove `import { Spacer } from "@sy-ui/react"`
...
1. **Remove Import**: Remove `Spacer` from `@sy-ui/react` imports
