...
**v2:** `<Snippet>` component from `@sy-ui/react`  
...
```tsx
import { Snippet } from "@sy-ui/react";
...
export default function App() {
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText("npm install @sy-ui/react");
    setCopied(true);
...
<Tabs items={["v2", "v3"]}>
  <Tab value="v2">
    ...
    <Snippet symbol="$">
      {[
        "npm install @sy-ui/react",
        "yarn add @sy-ui/react",
        "pnpm add @sy-ui/react"
      ]}
    </Snippet>
    ```
  </Tab>
...
2. **Import Change**: Remove `import { Snippet } from "@sy-ui/react"`
...
1. **Remove Import**: Remove `Snippet` from `@sy-ui/react` imports
