---
title: Snippet
description: Migration guide for Snippet from SY INC v2 to v3
---

<Callout type="warning">
  The Snippet component has been **removed** in SY INC v3. Use native HTML elements with Tailwind CSS classes and implement copy functionality manually using the Clipboard API.
</Callout>
...
**v2:** `<Snippet>` component from `@sy-inc/react`
...
```tsx
import { Snippet } from "@sy-inc/react";
...
export default function App() {
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText("npm install @sy-inc/react");
    setCopied(true);
...
<Tabs items={["v2", "v3"]}>
  <Tab value="v2">
    ...
    <Snippet symbol="$">
      {[
        "npm install @sy-inc/react",
        "yarn add @sy-inc/react",
        "pnpm add @sy-inc/react"
      ]}
    </Snippet>
    ```
  </Tab>
...
2. **Import Change**: Remove `import { Snippet } from "@sy-inc/react"`
...
1. **Remove Import**: Remove `Snippet` from `@sy-inc/react` imports
