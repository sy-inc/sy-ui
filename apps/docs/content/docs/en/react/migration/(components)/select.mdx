---
title: Select
description: Migration guide for Select from SY INC v2 to v3
---

<Callout type="info">
  Refer to the [v3 Select documentation](/docs/react/components/select) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from SY INC v2.
</Callout>
...
```tsx
import { Select, SelectItem } from "@sy-inc/react";
...
```tsx
import { Select, Label, ListBox } from "@sy-inc/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { Label, Description, FieldError } from "@sy-inc/react";
...
<Tabs items={["v2", "v3"]}>
  <Tab value="v2">
    ```tsx
    import { Select, SelectItem, SelectSection } from "@sy-inc/react";
  ...
  <Tab value="v3">
    ```tsx
    import { Select, Label, ListBox, Header, Separator } from "@sy-inc/react";
