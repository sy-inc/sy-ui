---
title: User
description: Migration guide for User from SY INC v2 to v3
---

<Callout type="warning">
  The User component has been **removed** in SY INC v3. Compose user displays manually using Avatar and text elements with Tailwind CSS classes.
</Callout>
...
**v2:** `<User>` component from `@sy-inc/react`
...
<Tabs items={["v2", "v3"]}>
  <Tab value="v2">
    ```tsx
    import { User, Link } from "@sy-inc/react";
...
2. **Import Change**: Remove `import { User } from "@sy-inc/react"`
...
1. **Remove Import**: Remove `User` from `@sy-inc/react` imports
