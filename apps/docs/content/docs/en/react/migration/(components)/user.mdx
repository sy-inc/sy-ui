...
**v2:** `<User>` component from `@sy-ui/react`  
...
<Tabs items={["v2", "v3"]}>
  <Tab value="v2">
    ```tsx
    import { User, Link } from "@sy-ui/react";
...
2. **Import Change**: Remove `import { User } from "@sy-ui/react"`
...
1. **Remove Import**: Remove `User` from `@sy-ui/react` imports
