---
title: RadioGroup
description: Migration guide for RadioGroup from SY INC v2 to v3
---

<Callout type="info">
  Refer to the [v3 RadioGroup documentation](/docs/react/components/radio-group) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from SY INC v2.
</Callout>
...
```tsx
import { RadioGroup, Radio } from "@sy-inc/react";
...
```tsx
import { RadioGroup, Radio, Label, Description } from "@sy-inc/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { RadioGroup, Radio, Label } from "@sy-inc/react";
