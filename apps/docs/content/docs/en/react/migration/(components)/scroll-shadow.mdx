---
title: ScrollShadow
description: Migration guide for ScrollShadow from SY INC v2 to v3
---

<Callout type="info">
  Refer to the [v3 ScrollShadow documentation](/docs/react/components/scroll-shadow) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from SY INC v2.
</Callout>
...
```tsx
import { ScrollShadow } from "@sy-inc/scroll-shadow";
...
**v2:** Imported from `@sy-inc/scroll-shadow` package
**v3:** Imported from `@sy-inc/react` unified package
...
- Update import from `@sy-inc/scroll-shadow` to `@sy-inc/react`
- Enhanced API with more configuration options
- Better shadow detection and visibility control
- Same orientation support (vertical / horizontal); import from `@sy-inc/react`
