---
title: ScrollShadow
description: Migration guide for ScrollShadow from SY UI v2 to v3
---

<Callout type="info">
  Refer to the [v3 ScrollShadow documentation](/docs/react/components/scroll-shadow) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from SY UI v2.
</Callout>
...
```tsx
import { ScrollShadow } from "@sy-ui/scroll-shadow";
...
**v2:** Imported from `@sy-ui/scroll-shadow` package  
...
- Update import from `@sy-ui/scroll-shadow` to `@sy-inc/react`
