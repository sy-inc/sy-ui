...
```tsx
import { Autocomplete, AutocompleteItem } from "@sy-ui/react";
...
```tsx
import { Autocomplete, Label, SearchField, ListBox, useFilter } from "@sy-ui/react";
...
The v3 Autocomplete uses the `useFilter` hook (from React Aria, re-exported by `@sy-ui/react`) to provide locale-aware filtering functions:

```tsx
import { useFilter } from "@sy-ui/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { Header, Separator } from "@sy-ui/react";
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ```tsx
    import { FieldError, Form } from "@sy-ui/react";
