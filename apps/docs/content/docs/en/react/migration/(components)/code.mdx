---
title: Code
description: Migration guide for Code from SY INC v2 to v3
---

<Callout type="warning">
  The Code component has been **removed** in SY INC v3. Use native HTML `<code>` element with Tailwind CSS classes instead.
</Callout>
...
**v2:** `<Code>` component from `@sy-inc/react`
...
```tsx
import { Code } from "@sy-inc/react";

export default function App() {
  return <Code>npm install @sy-inc/react</Code>;
}
```
    ...
    export default function App() {
      return (
        <div className="space-y-4">
          <p>
            Install SY INC with <Code>npm install @sy-inc/react</Code>
          </p>
...
2. **Import Change**: Remove `import { Code } from "@sy-inc/react"`
...
1. **Remove Import**: Remove `Code` from `@sy-inc/react` imports
