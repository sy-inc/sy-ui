...
**v2:** `<Code>` component from `@sy-ui/react`  
...
```tsx
import { Code } from "@sy-ui/react";

export default function App() {
  return <Code>npm install @sy-ui/react</Code>;
}
```
    ...
    export default function App() {
      return (
        <div className="space-y-4">
          <p>
            Install SY UI with <Code>npm install @sy-ui/react</Code>
          </p>
...
2. **Import Change**: Remove `import { Code } from "@sy-ui/react"`
...
1. **Remove Import**: Remove `Code` from `@sy-ui/react` imports
