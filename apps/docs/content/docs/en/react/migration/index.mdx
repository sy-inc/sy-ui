---
title: Migration
description: Complete guide to migrate your SY UI v2 application to v3
index: true
---

## For AI Assistants

Below are three approaches for AI assistants to access migration documentation. ***We recommend using the SY UI Migration MCP server*** and leveraging its prompts and tools, but all options provide agents with the complete documentation.
...
| **Setup** | MCP config | Install command | `sy-ui-cli` command |
...
- **Dependencies**: Update React to v19+, SY UI packages to v3, Tailwind CSS to v4, remove Framer Motion
- **No Provider Required**: v3 removes the need for `SyUIProvider`
...
- **Configuration**: Remove `sy-ui()` plugin from Tailwind config, update CSS imports, remove `hero.ts` file
...
SY UI v2 and v3 cannot coexist in the same project without special setup. You have two migration approaches to choose from:
...
<Callout>
  **Component Development Status**: Components marked with 🔄 In Progress or 📋 Planned are still being developed. Check the [Roadmap](https://sy-ui.featurebase.app/roadmap) for the task status. Guides for these components will be available once development is finished.
</Callout>
...
3. Check the [GitHub Discussions](https://github.com/sy-ui/sy-ui/discussions)
