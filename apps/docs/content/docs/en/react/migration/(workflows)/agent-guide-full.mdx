...
description: Full migration guide for AI assistants helping migrate SY UI v2 to v3
index: false
---

## Overview

This guide is designed for AI assistants (agents) helping users migrate from SY UI v2 to v3. It implements a **full migration approach** that analyzes the project first and migrates components in manageable batches rather than all at once.

<Callout type="warning">
  **Critical Constraint**: SY UI v2 and v3 **should not coexist** in the same project. The project will be broken during migration. Always work in a feature branch and migrate all component code before switching dependencies.
</Callout>
...
- Removing SyUIProvider
- Component imports and migration
- Hooks migration
- Styling migration
- Testing

**Agent Note**: Dependency updates (React 19, Tailwind v4) can be done before switching SY UI (won't break project). However, the SY UI package switch should only happen AFTER all component code is migrated.

## Hooks Migration

SY UI v2 provided component hooks (like `useSwitch`, `useInput`, `useCheckbox`, etc.) and utility hooks like `useDisclosure`. SY UI v3 removes most component hooks in favor of compound components, and replaces `useDisclosure` with `useOverlayState`.
...
<Callout type="info">
  **Removed/In-Progress/Planned Components**: For components marked as ❌ Removed, 🔄 In Progress, or 📋 Planned, replace them with standard HTML elements during migration. You can migrate back to SY UI components once they become available in v3.
</Callout>
...
These steps can be done before switching SY UI dependencies and won't break the project:

1. **Update React to v19** (if not already)
   - This can be done before switching SY UI
...
2. Switch from Migration MCP to `sy-ui-react` MCP for v3 development
