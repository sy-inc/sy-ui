...
description: Incremental coexistence migration guide for AI assistants helping migrate SY INC v2 to v3
index: false
---

## Overview

This guide is designed for AI assistants (agents) helping users migrate from SY INC v2 to v3 using **incremental coexistence migration**. This approach allows v2 and v3 components to work side-by-side, enabling component-by-component migration while keeping the project functional.
...
1. **Identify the strategy**: Check if project uses pnpm aliases (Strategy A) or component packages (Strategy B)
   - **Strategy A**: Look for aliases like `"@sy-inc-v3/react": "npm:@sy-inc/react@latest"` in package.json
   - **Strategy B**: Look for component-specific packages like `@sy-inc/button`, `@sy-inc/card` alongside `@sy-inc/react`

2. **Verify setup**: Ensure the coexistence setup is correct:
   - Strategy A: Both `@sy-inc/react` (v2) and `@sy-inc-v3/react` (v3 alias) are installed
   - Strategy B: `@sy-inc/react` (v3) and component packages like `@sy-inc/button` (v2) are installed
...
2. **Update imports**
   - Change imports from `@sy-inc/react` to `@sy-inc-v3/react`
   - Example: `import {Button} from "@sy-inc/react"` → `import {Button} from "@sy-inc-v3/react"`
...
2. **Remove component package**
   - Remove the v2 component package from dependencies (e.g., `@sy-inc/button`)
   - Update package.json

3. **Update imports**
   - Change imports from component package to `@sy-inc/react` (v3)
   - Example: `import {Card} from "@sy-inc/card"` → `import {Card} from "@sy-inc/react"`
...
1. **Remove v2 dependencies**
   - Strategy A: Remove `@sy-inc/react`, `@sy-inc/theme`, and aliases
   - Strategy B: Remove all remaining `@sy-inc/*` component packages
...
2. **Update all imports**
   - Strategy A: Change `@sy-inc-v3/react` → `@sy-inc/react`
   - Strategy B: All imports should already be `@sy-inc/react` (v3)
...
- Projects using unified `@sy-inc/react` package (Strategy A can work but full migration may be simpler)
