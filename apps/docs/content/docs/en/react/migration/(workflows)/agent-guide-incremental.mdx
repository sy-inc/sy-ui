...
1. **Identify the strategy**: Check if project uses pnpm aliases (Strategy A) or component packages (Strategy B)
   - **Strategy A**: Look for aliases like `"@sy-ui-v3/react": "npm:@sy-ui/react@latest"` in package.json
   - **Strategy B**: Look for component-specific packages like `@sy-ui/button`, `@sy-ui/card` alongside `@sy-ui/react`

2. **Verify setup**: Ensure the coexistence setup is correct:
   - Strategy A: Both `@sy-ui/react` (v2) and `@sy-ui-v3/react` (v3 alias) are installed
   - Strategy B: `@sy-ui/react` (v3) and component packages like `@sy-ui/button` (v2) are installed
...
2. **Update imports**
   - Change imports from `@sy-ui/react` to `@sy-ui-v3/react`
   - Example: `import {Button} from "@sy-ui/react"` → `import {Button} from "@sy-ui-v3/react"`
...
3. **Update imports**
   - Change imports from component package to `@sy-ui/react` (v3)
   - Example: `import {Card} from "@sy-ui/card"` → `import {Card} from "@sy-ui/react"`
...
1. **Remove v2 dependencies**
   - Strategy A: Remove `@sy-ui/react`, `@sy-ui/theme`, and aliases
...
2. **Update all imports**
   - Strategy A: Change `@sy-ui-v3/react` → `@sy-ui/react`
   - Strategy B: All imports should already be `@sy-ui/react` (v3)
...
- Projects using unified `@sy-ui/react` package (Strategy A can work but full migration may be simpler)
