...
- **Using `@sy-ui/react`**: Use Strategy A (pnpm aliases)
...
```json
{
  "dependencies": {
    "@sy-ui/react": "2.8.6",
    "@sy-ui/theme": "2.4.24",
    "@sy-ui-v3/react": "npm:@sy-ui/react@latest",
    "@sy-ui-v3/styles": "npm:@sy-ui/styles@latest"
  }
}
```

2. Import v2 components from `@sy-ui/react`:

```tsx
import {Button} from "@sy-ui/react"; // v2
```
...
3. **Once all components are migrated, switch to v3-only:**
   - Remove v2 dependencies (`@sy-ui/react`, `@sy-ui/theme`)
   - Remove aliases
   - Update all imports to `@sy-ui/react` (remove `-v3` suffix)
...
{
  "dependencies": {
    "@sy-ui/react": "latest", // v3
    "@sy-ui/styles": "latest", // v3
...
2. Import v3 components from `@sy-ui/react`:

```tsx
import {Button} from "@sy-ui/react"; // v3
```
...
<Callout type="warning">
  **Important:** Import order matters. Import `tailwindcss` first, then `@sy-ui/styles`.
</Callout>
...
2. **Update imports**
   - Strategy A: Change `@sy-ui/react` → `@sy-ui-v3/react`
   - Strategy B: Change component package → `@sy-ui/react`
...
2. **Guide conflict resolution**
   - Ensure CSS import order: `tailwindcss` first, then `@sy-ui/styles`
...
1. **Remove v2 dependencies:**
   - Remove `@sy-ui/react` and `@sy-ui/theme` (Strategy A)
...
2. **Update all imports:**
   - Change `@sy-ui-v3/react` → `@sy-ui/react` (Strategy A)
   - Change component package imports → `@sy-ui/react` (Strategy B)

3. **Update CSS:**
   - Remove v2 Tailwind plugin from config
   - Keep only `@import "@sy-ui/styles";`
