...
description: Step-by-step guide for migrating SY UI v2 to v3 incrementally while keeping both versions working side-by-side
---

## Overview

Incremental migration with coexistence allows you to migrate from SY UI v2 to v3 component-by-component while keeping your project functional throughout the migration process. This approach uses special setup strategies to allow both v2 and v3 components to work side-by-side.
...
- **Provider**: v2 requires `SyUIProvider`, v3 doesn't - you may need conditional provider wrapping
...
- **Using component packages** (`@sy-ui/button`, `@sy-ui/card`, etc.): Use Strategy B (component packages)
...
```json
{
  "dependencies": {
    "@sy-inc/react": "2.8.6",
    "@sy-ui/theme": "2.4.24",
    "@sy-ui-v3/react": "npm:@sy-inc/react@latest",
    "@sy-ui-v3/styles": "npm:@sy-inc/styles@latest"
  }
}
```
...
3. Import v3 components from `@sy-ui-v3/react`:

```tsx
import {Button} from "@sy-ui-v3/react"; // v3
```

### Migration Process

1. **Migrate one component at a time:**
   - Update imports to use `@sy-ui-v3/react`
...
3. **Once all components are migrated, switch to v3-only:**
   - Remove v2 dependencies (`@sy-inc/react`, `@sy-ui/theme`)
   - Remove aliases
...
```tsx
import {Card} from "@sy-ui/card"; // v2
```
...
2. **Update imports**
   - Strategy A: Change `@sy-inc/react` → `@sy-ui-v3/react`
...
1. **Remove v2 dependencies:**
   - Remove `@sy-inc/react` and `@sy-ui/theme` (Strategy A)
   - Remove all `@sy-ui/*` component packages (Strategy B)
...
2. **Update all imports:**
   - Change `@sy-ui-v3/react` → `@sy-inc/react` (Strategy A)
