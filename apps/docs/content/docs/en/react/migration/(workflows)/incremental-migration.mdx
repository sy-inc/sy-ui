...
description: Step-by-step guide for migrating SY INC v2 to v3 incrementally while keeping both versions working side-by-side
---

## Overview

Incremental migration with coexistence allows you to migrate from SY INC v2 to v3 component-by-component while keeping your project functional throughout the migration process. This approach uses special setup strategies to allow both v2 and v3 components to work side-by-side.
...
- **Provider**: v2 requires `SyIncProvider`, v3 doesn't - you may need conditional provider wrapping
...
- **Using `@sy-inc/react`**: Use Strategy A (pnpm aliases)
- **Using component packages** (`@sy-inc/button`, `@sy-inc/card`, etc.): Use Strategy B (component packages)
...
```json
{
  "dependencies": {
    "@sy-inc/react": "2.8.6",
    "@sy-inc/theme": "2.4.24",
    "@sy-inc-v3/react": "npm:@sy-inc/react@latest",
    "@sy-inc-v3/styles": "npm:@sy-inc/styles@latest"
  }
}
```

2. Import v2 components from `@sy-inc/react`:

```tsx
import {Button} from "@sy-inc/react"; // v2
```

3. Import v3 components from `@sy-inc-v3/react`:

```tsx
import {Button} from "@sy-inc-v3/react"; // v3
```

### Migration Process

1. **Migrate one component at a time:**
   - Update imports to use `@sy-inc-v3/react`
...
3. **Once all components are migrated, switch to v3-only:**
   - Remove v2 dependencies (`@sy-inc/react`, `@sy-inc/theme`)
   - Remove aliases
   - Update all imports to `@sy-inc/react` (remove `-v3` suffix)
...
{
  "dependencies": {
    "@sy-inc/react": "latest", // v3
    "@sy-inc/styles": "latest", // v3
    "@sy-inc/button": "2.8.6", // v2
    "@sy-inc/card": "2.8.6", // v2
    // ... other v2 component packages as needed
  }
}
```

2. Import v3 components from `@sy-inc/react`:

```tsx
import {Button} from "@sy-inc/react"; // v3
```

3. Import v2 components from component packages:

```tsx
import {Card} from "@sy-inc/card"; // v2
```

### Migration Process

1. **Install v3 packages** (`@sy-inc/react`, `@sy-inc/styles`)
2. **Install v2 component packages** for components not yet migrated
3. **Migrate one component at a time:**
   - Remove the v2 component package from dependencies
   - Update imports to use `@sy-inc/react` (v3)
   - Update component code to v3 API
   - Test the migrated component
   - For v2 components with no v3 counterpart (Code, Image, Navbar, Ripple, Snippet, Spacer, User), leave them in place. They will continue to work during coexistence.
...
<Callout type="warning">
  **Important:** Import order matters. Import `tailwindcss` first, then `@sy-inc/styles`.
</Callout>
...
2. **Update imports**
   - Strategy A: Change `@sy-inc/react` → `@sy-inc-v3/react`
   - Strategy B: Change component package → `@sy-inc/react`
...
2. **Guide conflict resolution**
   - Ensure CSS import order: `tailwindcss` first, then `@sy-inc/styles`
...
1. **Remove v2 dependencies:**
   - Remove `@sy-inc/react` and `@sy-inc/theme` (Strategy A)
   - Remove all `@sy-inc/*` component packages (Strategy B)
...
2. **Update all imports:**
   - Change `@sy-inc-v3/react` → `@sy-inc/react` (Strategy A)
   - Change component package imports → `@sy-inc/react` (Strategy B)

3. **Update CSS:**
   - Remove v2 Tailwind plugin from config
   - Keep only `@import "@sy-inc/styles";`
