...
description: Step-by-step guide for migrating SY UI v2 to v3 using full migration approach
---

## Overview

Full migration is a structured approach to migrating from SY UI v2 to v3. This method involves migrating all component code first, then switching dependencies, ensuring a clean transition.
...
**Note**: These dependency updates can be done before switching SY UI (won't break project). However, the SY UI package switch should only happen AFTER all component code is migrated.

#### Update SY UI Packages
...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
    ```bash
    npm uninstall @sy-inc/react @sy-ui/theme
    npm install @sy-inc/styles @sy-inc/react
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm remove @sy-inc/react @sy-ui/theme
    pnpm add @sy-inc/styles @sy-inc/react
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn remove @sy-inc/react @sy-ui/theme
    yarn add @sy-inc/styles @sy-inc/react
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun remove @sy-inc/react @sy-ui/theme
...
```js
// tailwind.config.js
const {sy-ui} = require("@sy-inc/react");

module.exports = {
  content: [
    "./src/**/*.{js,ts,jsx,tsx}",
    "./node_modules/@sy-ui/theme/dist/**/*.{js,ts,jsx,tsx}",
  ],
  plugins: [sy-ui()],
  // ... other config
};
...
Remove the `sy-ui()` plugin from your Tailwind config. If you only used Tailwind for SY UI and have no other customizations, you can remove `tailwind.config.js` entirely. Otherwise, keep the file but remove the SY UI plugin configuration.
...
<section id="step-3-remove-sy-uiprovider">

### Step 3: Remove SyUIProvider
...
import {Button} from "@sy-ui/button";
import {Card} from "@sy-ui/card";
...
### Dependencies
- Update React to v19+
- Update SY UI packages to v3 (after component migration)
- Remove Framer Motion
- Update Tailwind CSS to v4

### Configuration
- Remove `sy-ui()` plugin from Tailwind config
- Update CSS imports
- Remove `hero.ts` file (if exists)

### Application Code - Component Migration
- Remove `SyUIProvider` wrapper
