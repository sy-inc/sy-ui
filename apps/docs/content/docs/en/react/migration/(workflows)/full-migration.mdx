...
description: Step-by-step guide for migrating SY INC v2 to v3 using full migration approach
---

## Overview

Full migration is a structured approach to migrating from SY INC v2 to v3. This method involves migrating all component code first, then switching dependencies, ensuring a clean transition.
...
**Note**: These dependency updates can be done before switching SY INC (won't break project). However, the SY INC package switch should only happen AFTER all component code is migrated.

#### Update SY INC Packages
...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
    ```bash
    npm uninstall @sy-inc/react @sy-inc/theme
    npm install @sy-inc/styles @sy-inc/react
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm remove @sy-inc/react @sy-inc/theme
    pnpm add @sy-inc/styles @sy-inc/react
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn remove @sy-inc/react @sy-inc/theme
    yarn add @sy-inc/styles @sy-inc/react
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun remove @sy-inc/react @sy-inc/theme
...
```js
// tailwind.config.js
const {sy-inc} = require("@sy-inc/react");

module.exports = {
  content: [
    "./src/**/*.{js,ts,jsx,tsx}",
    "./node_modules/@sy-inc/theme/dist/**/*.{js,ts,jsx,tsx}",
  ],
  plugins: [sy-inc()],
  // ... other config
};
...
Remove the `sy-inc()` plugin from your Tailwind config. If you only used Tailwind for SY INC and have no other customizations, you can remove `tailwind.config.js` entirely. Otherwise, keep the file but remove the SY INC plugin configuration.
...
/* globals.css or main.css */
@import "tailwindcss";
@import "@sy-inc/styles"; /* [!code highlight]*/
```

<Callout type="warning">
  Import order matters! Always import `tailwindcss` before `@sy-inc/styles`.
</Callout>
...
<section id="step-3-remove-sy-incprovider">

### Step 3: Remove SyIncProvider
...
import {Button} from "@sy-inc/button";
import {Card} from "@sy-inc/card";
...
import {Button, type ButtonProps} from "@sy-inc/react";

// Compound component types are properly exported
import {Checkbox, type CheckboxProps} from "@sy-inc/react";
...
### Dependencies
- Update React to v19+
- Update SY INC packages to v3 (after component migration)
- Remove Framer Motion
- Update Tailwind CSS to v4

### Configuration
- Remove `sy-inc()` plugin from Tailwind config
- Update CSS imports
- Remove `hero.ts` file (if exists)

### Application Code - Component Migration
- Remove `SyIncProvider` wrapper
- Handle Provider props migration (router, locale, animations)
- Update all component imports to `@sy-inc/react`
