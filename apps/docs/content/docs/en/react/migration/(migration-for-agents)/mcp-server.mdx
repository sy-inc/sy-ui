---
title: MCP Server
description: Access SY UI v2 to v3 migration guides in your AI assistant
---

The SY UI Migration MCP Server gives AI assistants direct access to SY UI v2 to v3 migration documentation.

The Migration MCP server uses **Streamable HTTP transport** (URL-based) and is deployed at `https://migration-mcp.sy-ui.com`. View the source code on [GitHub](https://github.com/sy-ui/sy-ui-mcp/tree/main/apps/migration-mcp).

<Callout>
  The Migration MCP is for **migration only**. After completing migration, switch to the [SY UI React MCP Server](/docs/react/getting-started/mcp-server) for v3 component development.
</Callout>
...
After adding the configuration, restart Claude Code and run `/mcp` to see the SY UI Migration MCP server in the list. If you see **Connected**, you're ready to use it.
...
After adding the configuration, open `.vscode/mcp.json` and click **Start** next to the sy-ui-migration server.
...
- "Help me migrate Button from SY UI v2 to v3"
...
| Prompt | Description |
|--------|-------------|
| `analyze-and-plan` | Analyze the project to identify all SY UI v2 components and create a migration plan. Use this as the first step in migration. Optional: `migrationType: "full"` (default) or `"incremental"` |
...
- [GitHub Repository](https://github.com/sy-ui/sy-ui-mcp/tree/main/apps/migration-mcp)
