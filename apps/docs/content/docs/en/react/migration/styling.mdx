...
description: Complete guide to styling changes and theming system migration from SY UI v2 to v3
---

<Callout type="info">
  This guide covers all styling-related changes between SY UI v2 and v3, including utility classes, component styles, theme system architecture, and visual differences. For component-specific API changes, see individual component migration guides.
</Callout>
...
SY UI v3 introduces significant changes to the styling system:
...
SY UI v2 provided custom text size utilities that mapped to CSS variables. v3 uses standard Tailwind text size classes.
...
If you only use SY UI, you can remove `tailwind.config.js` entirely. If you have custom Tailwind config, keep it but remove the SY UI plugin.
...
v2 used the pattern `--sy-ui-{property}-{scale}` while v3 uses `--{property}` or `--color-{property}`.

**v2 CSS Variables:**

```css
--sy-ui-font-size-tiny: 0.75rem;
--sy-ui-font-size-small: 0.875rem;
--sy-ui-radius-small: 8px;
--sy-ui-radius-medium: 12px;
--sy-ui-border-width-medium: 2px;
--sy-ui-disabled-opacity: 0.5;
```
...
```css
--sy-ui-primary: 210 100% 50%;
--sy-ui-primary-50: 210 100% 95%;
--sy-ui-primary-100: 210 100% 90%;
/* ... more shades ... */
```
...
```css
--sy-ui-divider-weight: 1px;
--sy-ui-disabled-opacity: 0.5;
--sy-ui-hover-opacity: 0.8;
```
...
```css
--sy-ui-box-shadow-small: 0px 0px 5px 0px rgb(0 0 0 / 0.02), ...;
--sy-ui-box-shadow-medium: 0px 0px 15px 0px rgb(0 0 0 / 0.03), ...;
--sy-ui-box-shadow-large: 0px 0px 30px 0px rgb(0 0 0 / 0.04), ...;
```
