...
description: Complete guide to styling changes and theming system migration from SY INC v2 to v3
---

<Callout type="info">
  This guide covers all styling-related changes between SY INC v2 and v3, including utility classes, component styles, theme system architecture, and visual differences. For component-specific API changes, see individual component migration guides.
</Callout>
...
SY INC v3 introduces significant changes to the styling system:
...
SY INC v2 provided custom text size utilities that mapped to CSS variables. v3 uses standard Tailwind text size classes.
...
v2 provided `.scrollbar-hide` and `.scrollbar-default` utilities. v3 now exposes standards-only scrollbar utilities from `@sy-inc/styles`: `scrollbar`, `scrollbar-thin`, `scrollbar-default`, and `scrollbar-none`. Use `data-scrollbar="thin"`, `data-scrollbar="default"`, or `data-scrollbar="none"` on an ancestor for subtree-level control.
...
If you only use SY INC, you can remove `tailwind.config.js` entirely. If you have custom Tailwind config, keep it but remove the SY INC plugin.
...
v2 used the pattern `--sy-inc-{property}-{scale}` while v3 uses `--{property}` or `--color-{property}`.

**v2 CSS Variables:**

```css
--sy-inc-font-size-tiny: 0.75rem;
--sy-inc-font-size-small: 0.875rem;
--sy-inc-radius-small: 8px;
--sy-inc-radius-medium: 12px;
--sy-inc-border-width-medium: 2px;
--sy-inc-disabled-opacity: 0.5;
```
...
```css
--sy-inc-primary: 210 100% 50%;
--sy-inc-primary-50: 210 100% 95%;
--sy-inc-primary-100: 210 100% 90%;
/* ... more shades ... */
```
...
```css
--sy-inc-divider-weight: 1px;
--sy-inc-disabled-opacity: 0.5;
--sy-inc-hover-opacity: 0.8;
```
...
```css
--sy-inc-box-shadow-small: 0px 0px 5px 0px rgb(0 0 0 / 0.02), ...;
--sy-inc-box-shadow-medium: 0px 0px 15px 0px rgb(0 0 0 / 0.03), ...;
--sy-inc-box-shadow-large: 0px 0px 30px 0px rgb(0 0 0 / 0.04), ...;
```
