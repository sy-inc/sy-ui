...
`Table.ManagedColumns` is the single geometry boundary for fixed, flexible, resizable, and pinned native columns. Its `columns` must list every leaf `Table.Column` by the same `id` and in the same order. Native pinning uses logical `start`/`end` edges, supports RTL, and adds a shadow only at a covered scroll edge. `Table.Summary` shares the same column track; set `isSticky` to keep it at the bottom of a constrained scrolling container.

<ComponentPreview
  name="table-managed-columns"
  minHeight="420px"
/>

Use `Table.Overflow` inside a cell to opt into truncation. Add `tooltip` to show the full value only when it is actually clipped. `Table.LoadingOverlay` supplies layout only: keep loading state and `aria-busy` on the owning application surface.
...
`Table.Virtualizer` composes the React Aria [Virtualizer](https://react-aria.adobe.com/Virtualizer) with managed column geometry, so virtual rows use the same widths and resizing state. Pass row sizing through `layoutOptions`; column widths come only from `columns` and cannot be overridden there. Pinned columns and `Table.Summary` are native-table features and are not supported in virtualized mode.
...
SY INC's Table works as a rendering layer on top of headless table libraries.
This example uses [TanStack Table](https://tanstack.com/table) for column
definitions, sorting, and pagination — while SY INC handles styling and
accessibility.
...
The Table component uses these CSS classes ([View source styles](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/components/table.css)):
...
- `.table__summary` - Native table footer row group (`<tfoot>`)
...
- `.table__overflow` - Opt-in truncation wrapper
- `.table__loading-overlay` - Presentational loading layer
...
| `indicator` | `React.ReactNode` | - | Custom indicator element. Overrides the default chevron and receives a `data-direction` attribute. Pass `null` to render no indicator. |
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `columns` | `TableColumnDefinition[]` | - | Ordered leaf definitions with `id` plus `width`, `flex`, `minWidth`, `maxWidth`, or native `pinned: "start" \| "end"`. |
| `fillContainer` | `boolean` | `false` | Distribute spare viewport width between columns. |
| `viewportWidth` | `number` | measured | Controlled geometry viewport width. Usually omit it. |

Place `Table.ResizableContainer` inside this boundary to feed React Aria resize results back into the same geometry. Pinning is native-only; an invalid pinned width emits a development warning.

### Table.Virtualizer

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `columns` | `TableColumnDefinition[]` | - | The authoritative virtual column geometry. |
| `layoutOptions` | `Omit<TableLayoutProps, "columnWidths">` | - | Row and heading layout options; managed widths cannot be bypassed. |
| `fillContainer` | `boolean` | `false` | Distribute spare viewport width between columns. |
| `viewportWidth` | `number` | measured | Controlled geometry viewport width. Usually omit it. |

### Table.Summary

Native `<tfoot>` row group. Render it inside `Table.Content`, after `Table.Body`; it is distinct from the external `Table.Footer` pagination area.

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `isSticky` | `boolean` | `true` | Stick the summary to the bottom of the nearest scrolling table viewport. Set to `false` to disable it. |
| `children` | `React.ReactNode` | - | `Table.Row` summary rows. |

### Table.Overflow

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `tooltip` | `boolean` | `false` | Enable the tooltip only while the content is measurably clipped. |
| `children` | `React.ReactNode` | - | Cell or header text. |

### Table.LoadingOverlay

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `children` | `React.ReactNode` | - | Presentational loading content. The component does not own loading state. |
