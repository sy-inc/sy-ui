...
```tsx
import { ColorField, parseColor } from '@sy-ui/react';
```
...
```tsx
import {ColorField, Label, ColorSwatch, Description, FieldError, parseColor} from '@sy-ui/react';
...
- **Label** - Field label component from `@sy-ui/react`
...
- **ColorSwatch** - Color preview component from `@sy-ui/react`
- **Description** - Helper text component from `@sy-ui/react`
- **FieldError** - Validation error message from `@sy-ui/react`
...
```tsx
import {parseColor} from '@sy-ui/react';
