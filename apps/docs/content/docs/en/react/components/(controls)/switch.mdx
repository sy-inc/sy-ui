...
```tsx
import { Switch, SwitchGroup, Label } from '@sy-inc/react';
```
...
Render the switch as a full-width settings row with `variant="cell"`. The whole
row is the toggle target. Use `variant="cell-secondary"` on a `default` surface.
...
The Switch component uses these CSS classes ([View source styles](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/components/switch.css)):
...
- `.switch--cell` - Full-width settings row variant
- `.switch--cell-secondary` - Settings row on a `default` surface

#### SwitchGroup Classes [!toc]

The SwitchGroup component uses these CSS classes ([View source styles](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/components/switch-group.css)):
...
| `variant` | `'default' \| 'cell' \| 'cell-secondary'` | `'default'` | Renders the switch as a full-width settings row |
