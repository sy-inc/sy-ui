...
```tsx
import { Slider } from '@sy-inc/react';
```
...
```tsx
import { Slider, Label } from '@sy-inc/react';
...
The Slider component uses these CSS classes ([View source styles](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/components/slider.css)):
