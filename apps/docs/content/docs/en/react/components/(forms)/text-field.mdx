...
```tsx
import { TextField } from '@sy-inc/react';
```
...
```tsx
import {TextField, Label, Input, Description, FieldError} from '@sy-inc/react';
...
| `fullWidth` | `boolean` | `false` | Whether the text field should take full width of its container. Propagates to Input, TextArea and InputGroup children; no need to repeat it on the child. |
| `variant` | `"primary" \| "secondary"` | `"primary"` | Visual variant. Propagates to Input, TextArea and InputGroup children; no need to repeat it on the child. |
...
- **Label** - Field label component from `@sy-inc/react`
- **Input** - Single-line text input from `@sy-inc/react`
- **TextArea** - Multi-line text input from `@sy-inc/react`
- **Description** - Helper text component from `@sy-inc/react`
- **FieldError** - Validation error message from `@sy-inc/react`
