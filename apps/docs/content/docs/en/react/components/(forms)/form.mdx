...
```tsx
import {Form, Button} from '@sy-inc/react';
...
`Form` renders a native `<form>` element and focuses on validation and submission. It does not include dedicated BEM classes in `@sy-inc/styles`—apply container styles from global CSS, and use field components for control-level customization.
...
SY INC follows the [BEM](https://getbem.com/) methodology for components that ship styles in `@sy-inc/styles`.
...
The Form component works seamlessly with SY INC's form field components:
