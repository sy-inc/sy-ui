...
```tsx
import {Form, Button} from '@sy-ui/react';
...
`Form` renders a native `<form>` element and focuses on validation and submission. It does not include dedicated BEM classes in `@sy-ui/styles`—apply container styles from global CSS, and use field components for control-level customization.
...
SY UI follows the [BEM](https://getbem.com/) methodology for components that ship styles in `@sy-ui/styles`.
