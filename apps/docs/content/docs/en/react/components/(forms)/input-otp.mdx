...
Use the `pattern` prop to restrict input to specific characters. SY UI exports common patterns like `REGEXP_ONLY_CHARS` and `REGEXP_ONLY_DIGITS`.
...
The InputOTP component uses these CSS classes ([View source styles](https://github.com/sy-ui/sy-ui/blob/v3/packages/styles/components/input-otp.css)):
...
SY UI re-exports common regex patterns from input-otp for convenience:
