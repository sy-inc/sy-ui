...
```tsx
import { InputGroup } from '@sy-inc/react';
```
...
```tsx
import {InputGroup, TextField, Label} from '@sy-inc/react';
...
export default () => (
  <TextField>
    ...
    <InputGroup>
      ...
      <InputGroup.Suffix>
        <InputGroup.PasswordToggle /> {/* Optional, with InputGroup.Input type="password" */}
      </InputGroup.Suffix>
    </InputGroup>
  </TextField>
)
...
Drop the built-in `InputGroup.PasswordToggle` into the suffix. `InputGroup` owns the visibility state, so the consumer keeps none: an `InputGroup.Input` with `type="password"` renders as `text` while the toggle is on.
...
```tsx
import {InputGroup, TextField, Label, Button} from '@sy-inc/react';
...
import {Description, FieldError, InputGroup, Label, TextField} from "@sy-inc/react";
...
- `[data-slot="input-group-password-toggle"]` – The built-in password visibility button inside the suffix. Zeroes `--button-bg-hover` and `--button-bg-pressed` so it stays transparent.
...
- **InputGroup.PasswordToggle** - Built-in show/hide password button
...
#### InputGroup.PasswordToggle Props

Renders a transparent icon-only [Button](/docs/components/button) — no background change on hover or press — that flips the group's password visibility. Place it inside `InputGroup.Suffix` — the suffix drops its padding and the button takes the full 40px, matching a plain-icon suffix, so no `className` is needed — and give the `InputGroup.Input` it controls `type="password"` — other input types are left untouched. Visibility is uncontrolled and owned by `InputGroup`.
