...
```tsx
import { Autocomplete, useFilter } from "@sy-inc/react";
```
...
```tsx
import {Autocomplete, Label, Description, SearchField, ListBox} from "@sy-inc/react";
...
For richer layouts such as removable tags, render your own content from `state.selectedItems` as
...
The Autocomplete component uses these CSS classes ([View source styles](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/components/autocomplete.css)):
