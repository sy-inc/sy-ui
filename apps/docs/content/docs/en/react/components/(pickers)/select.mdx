...
```tsx
import { Select } from "@sy-inc/react";
```
...
```tsx
import {Select, Label, Description, Header, ListBox, Separator} from "@sy-inc/react";
...
The Select component uses these CSS classes ([View source styles](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/components/select.css)):
