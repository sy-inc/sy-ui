...
```tsx
import { ComboBox } from '@sy-inc/react';
```
...
```tsx
import { ComboBox, Input, Label, Description, Header, ListBox, Separator } from '@sy-inc/react';
...
The ComboBox component uses these CSS classes ([View source styles](https://github.com/sy-inc/sy-inc/blob/v3/packages/styles/components/combo-box.css)):
...
```tsx
import { ComboBox, Input, Label, ListBox, Header, Separator } from '@sy-inc/react';
...
```tsx
import { Collection, ComboBox, EmptyState, Input, Label, ListBox, ListBoxLoadMoreItem, Spinner } from '@sy-inc/react';
...
```tsx
import { ComboBox, FieldError, Input, Label, ListBox } from '@sy-inc/react';
