...
```tsx
import { TimeField } from '@sy-inc/react';
```
...
```tsx
import {TimeField, Label, Description, FieldError} from '@sy-inc/react';
...
- **TimeField.Input** - Input component with segmented editing from `@sy-inc/react`
