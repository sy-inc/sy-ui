...
```tsx
import { TimeField } from '@sy-ui/react';
```
...
```tsx
import {TimeField, Label, Description, FieldError} from '@sy-ui/react';
...
- **TimeField.Input** - Input component with segmented editing from `@sy-ui/react`
