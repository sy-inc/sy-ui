...
```tsx
import { DateField } from '@sy-ui/react';
```
...
```tsx
import {DateField, Label, Description, FieldError} from '@sy-ui/react';
...
- **DateField.Input** - Input component with segmented editing from `@sy-ui/react`
