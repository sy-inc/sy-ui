...
```tsx
import { DateField } from '@sy-inc/react';
```
...
```tsx
import {DateField, Label, Description, FieldError} from '@sy-inc/react';
...
- **DateField.Input** - Input component with segmented editing from `@sy-inc/react`
