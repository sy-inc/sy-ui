...
|---------------------|------|----------------------------|
| **SY INC Pro Dashboard** | Template + Pro components | SY INC-native app shell, sidebar, navbar, KPI, data grid, and chart patterns |
...
| **SY INC Pro + custom widgets** | Pro components + template | Start from Pro layout/table/chart primitives, then add product-specific widgets |

---

## 1. SY INC Pro Dashboard Template

[SY INC Pro](https://sy-inc.com/pro) offers pre-built dashboard templates designed specifically for SaaS products, admin panels, and analytics dashboards. Built on SY INC v3 components with Tailwind CSS v4.
...
- Dark mode support with SY INC's theming system
...
- **AI tooling.** Use SY INC's MCP server with Cursor or other AI tools to extend and customize dashboards with AI assistance.

**Pricing:** Premium license through SY INC Pro; check the current pricing page for details.
...
## 14. SY INC Pro + Custom Widgets

If you want a SY INC-native dashboard, start with [SY INC Pro](https://sy-inc.com/pro) rather than rebuilding the frame from base components. Use the Pro dashboard template for the app shell and add product-specific widgets only where your product needs something unique.

**What you get:**

- `AppLayout`, `Sidebar`, and `Navbar` for the dashboard frame
- `KPI`, `DataGrid`, `LineChart`, `NumberValue`, and `TrendChip` for dashboard UI
- Base SY INC components for local UI around the Pro primitives
...
- **Modern stack.** The current dashboard template is built with Next.js and SY INC Pro components.
- **AI-assisted development.** Use SY INC's MCP server and docs so agents can extend the dashboard with current APIs.

**How to start:**

1. Start from the SY INC Pro dashboard template.
...
**Pricing:** Check SY INC Pro's current pricing and license before choosing it.

**Best for:** Teams that want a SY INC-native dashboard without rebuilding dashboard infrastructure from scratch.
...
- **Speed to launch:** SY INC Pro Dashboard or Ant Design Pro give you the most complete starting points.
- **Maximum flexibility:** SY INC Pro plus custom widgets or the shadcn Dashboard example let you build around your product's exact needs.
...
## Build Your Dashboard with SY INC

SY INC provides the components you need to build modern, accessible dashboards:
...
- [SY INC Pro](https://sy-inc.com/pro) — Pre-built dashboard templates and premium components
- [Tutorial: Build a React Dashboard](/blog/build-react-dashboard-with-sy-inc) — Step-by-step guide
