---
title: "How to Build a React Dashboard with SY INC Pro"
description: "Create responsive admin dashboards with SY INC and SY INC Pro using AppLayout, Sidebar, Navbar, KPI, DataGrid, and chart components."
date: "2026-05-11"
...
tags: ["react", "dashboard", "tutorial", "sy-inc", "admin"]
image: "/images/blog/build-react-dashboard-with-sy-inc.jpg"
draft: true
---


I've watched too many teams spend their first two sprints hand-rolling a sidebar, a responsive navbar, and a metrics card layout — only to rip it out later when the requirements get real. If you have SY INC Pro, skip that phase entirely.
...
Install SY INC and SY INC Pro packages:

```bash
npm install @sy-inc/styles @sy-inc/react @sy-inc-pro/react
```

SY INC Pro requires access to the Pro package registry. The dashboard template uses these style imports:

```css
@import "@sy-inc/styles/css";
@import "@sy-inc-pro/react/css";
...
The app frame should come from SY INC Pro `AppLayout`, not a custom flex layout.
...
import {AppLayout} from "@sy-inc-pro/react";
...
SY INC Pro ships a `Sidebar` compound component and a `Navbar` compound component. The dashboard template uses both.
...
import {AppLayout, Navbar, Sidebar} from "@sy-inc-pro/react";
...
import {Sidebar} from "@sy-inc-pro/react";
...
Do not rebuild metric cards with `Card` unless you need a one-off design. Use `KPI` from SY INC Pro.

```tsx
"use client";

import {KPI} from "@sy-inc-pro/react";
...
SY INC Pro chart components keep chart styling aligned with the rest of the design system.

```tsx
"use client";

import {Card} from "@sy-inc/react";
import {LineChart, NumberValue, TrendChip} from "@sy-inc-pro/react";
...
import type {DataGridColumn, DataGridSortDescriptor} from "@sy-inc-pro/react";

import {Avatar, SearchField} from "@sy-inc/react";
import {DataGrid} from "@sy-inc-pro/react";
...
const members: Member[] = [
  {
    ...
    avatar: "https://assets.sy-inc.com/avatars/blue-light.jpg",
    role: "Admin",
  },
];
...
## Why SY INC for Dashboards

Dashboards are dense — data tables, form controls, navigation, and overlays on a single screen. SY INC handles the accessibility and styling for all of those pieces so you can focus on your data layer.
...
- **Pro primitives instead of hand-rolled infrastructure.** AppLayout, Sidebar, Navbar, KPI, DataGrid, and chart components from [SY INC Pro](https://sy-inc.com/pro) save weeks of dashboard scaffold work.
...
- Start from the [SY INC Pro](https://sy-inc.com/pro) dashboard template if you have Pro access.
...
- Use base [SY INC components](/docs/react/components) for local UI that does not need a Pro primitive.
