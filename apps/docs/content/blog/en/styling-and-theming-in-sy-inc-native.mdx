---
title: "How Styling and Theming Work in SY INC Native"
description: "Why SY INC Native ships real CSS files for React Native, how its design tokens derive states automatically, and how Pro themes change components without forking them."
date: "2026-07-31"
...
image: "https://assets.sy-inc.com/blog/sy-inc-native-styling.jpg"
---
...
Notably, this does not go through a React theme provider. Uniwind resolves the variables, so a theme change does not cascade a context update down your entire tree. `SyIncNativeProvider` exists, but it handles safe-area insets, text defaults, the toast host, and RTL — it is app infrastructure, not a theme engine.

## The Pro Theme Pattern

Because tokens are variables and component styles are CSS classes, a complete visual identity can be packaged as a stylesheet. That is how SY INC Native Pro ships themes, and it works in two passes. The Brutalism theme shows both clearly, since it is about as far from the default look as a theme can get.
...
```css
@import "sy-inc-native/styles";
@import "sy-inc-native-pro/styles";
@import "sy-inc-native-pro/themes/brutalism";
```
...
- **The vocabulary matches the rest of SY INC.** Semantic roles, foreground pairing, and compound components are the same ideas you already use on the web.
