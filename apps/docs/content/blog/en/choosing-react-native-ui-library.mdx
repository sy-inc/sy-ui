...
description: "A practical framework for choosing a React Native UI library, and where SY UI Native fits."
date: "2026-05-12"
...
This guide explains how we think about that decision, and why SY UI Native is built the way it is.
...
SY UI Native takes the package approach:

- Install `sy-ui-native`.
...
SY UI Native is intentionally opinionated about the baseline:
...
SY UI Native is strongest when you want one ecosystem across product surfaces:

- Web product: [SY UI React](/docs/react/getting-started/quick-start)
- Mobile product: [SY UI Native](/docs/native/getting-started)
- Premium app surfaces: [SY UI Pro](https://sy-ui.com/pro)
- Design handoff: [SY UI Figma Kit V3](https://www.figma.com/community/file/1546526812159103429/sy-ui-figma-kit-v3)
...
## Where SY UI Native Is Different

SY UI Native is built specifically for React Native. It does not ask you to use web imports or CSS files that do not belong in mobile apps.
...
Do not use the SY UI web package or web-only event handlers in React Native screens.

SY UI Native uses `onPress`, `SyUINativeProvider`, `GestureHandlerRootView`, Uniwind, and native overlay/portal patterns because those are the right abstractions for mobile.

## Recommendation

Choose SY UI Native when your team values design quality, maintainability, and ecosystem leverage. It is especially compelling if you already use SY UI on the web, work from Figma, or build with AI coding assistants.
