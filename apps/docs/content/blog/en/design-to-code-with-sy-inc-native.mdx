---
title: "Design to Code with SY INC Native"
description: "How SY INC Native connects Figma, code, and AI-assisted mobile development."
date: "2026-05-13"
...
image: "/images/blog/design-to-code-with-sy-inc-native.jpg"
draft: true
---
...
SY INC Native is built for that workflow. The library gives developers a maintained React Native component system, gives designers a shared SY INC visual language through Figma, and gives AI assistants structured documentation through MCP, llms.txt, and skills.
...
The result is subtle inconsistency. One screen has different spacing. Another uses the wrong disabled state. A generated component imports web SY INC into a native app. The first version ships, but the system becomes harder to maintain.

SY INC Native is designed to reduce that drift.

## Shared Mental Model

SY INC Native uses the same product-level ideas as the rest of the SY INC ecosystem:
...
The implementation is native, not web. SY INC Native uses React Native, Uniwind, `SyIncNativeProvider`, and mobile interaction patterns. But the design language stays familiar.
...
The [SY INC Figma Kit V3](https://www.figma.com/community/file/1546526812159103429/sy-inc-figma-kit-v3) is the design-side entry point for the ecosystem. It is linked from the Native docs so teams can work from the same visual system while building mobile screens.

Use it to:

- Start from SY INC's visual language instead of blank frames.
- Keep component names and variants close to the code model.
- Align designers and developers on hierarchy, states, and spacing.
- Create mobile product screens that feel connected to the rest of the SY INC ecosystem.
...
SY INC Native provides dedicated AI entry points:
...
- [SY INC Native MCP Server](/docs/native/getting-started/mcp-server), published as `@sy-inc/native-mcp`.
- [SY INC Native Agent Skills](/docs/native/getting-started/agent-skills).

That matters because Native code has different rules:

- Import from `sy-inc-native`.
- Use React Native press handlers such as `onPress`.
- Configure `SyIncNativeProvider`.
- Wrap the app with `GestureHandlerRootView`.
- Use Uniwind and `sy-inc-native/styles`.
...
1. **Design from the Figma kit.** Start from SY INC's component language instead of one-off mobile components.
2. **Install SY INC Native correctly.** Follow the [Quick Start](/docs/native/getting-started/quick-start) so Uniwind, peer dependencies, styles, and providers are configured.
...
6. **Update through the package.** Treat SY INC Native as a maintained dependency, not a copied snapshot.

## Why This Helps

Design-to-code is not about removing engineers or designers. It is about reducing translation loss.

SY INC Native helps because:

- Designers start from the SY INC visual system.
...
Install the [SY INC Native MCP Server](/docs/native/getting-started/mcp-server) in your editor, then ask your assistant to build a screen using the [Quick Start](/docs/native/getting-started/quick-start) and the components in [All Components](/docs/native/components).
