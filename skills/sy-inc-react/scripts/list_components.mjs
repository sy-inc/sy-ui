#!/usr/bin/env node
/**
 * List all available SY INC v3 components.
 *
 * Usage:
...
async function fetchFallback() {
  try {
    ...
    for (const line of content.split("\n")) {
      ...
      // Match: - [ComponentName](https://www.sy-inc.com/docs/react/components/component-name)
      // Skip "All Components" which links to /components without a specific component
      if (inComponentsSection) {
        const match = line.match(
          /^\s*-\s*\[([^\]]+)\]\(https:\/\/www\.sy-inc\.com\/docs\/react\/components\/[a-z]/,
        );
