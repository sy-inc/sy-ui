#!/usr/bin/env node
/**
 * List all available SY INC Native components.
 *
 * Usage:
...
const API_BASE = process.env.SY_INC_NATIVE_API_BASE || "https://native-mcp-api.sy-inc.com";
const APP_PARAM = "app=native-skills";
const LLMS_TXT_URL = "https://sy-inc.com/native/llms.txt";

/**
 * Fetch data from SY INC Native API with app parameter for analytics.
 */
async function fetchApi(endpoint) {
  ...
  try {
    const response = await fetch(url, {
      headers: {"User-Agent": "SY INC-Native-Skill/1.0"},
...
async function fetchFallback() {
  try {
    ...
    for (const line of content.split("\n")) {
      ...
      // Match: - [ComponentName](https://www.sy-inc.com/docs/native/components/component-name)
      // Skip "All Components" which links to /components without a specific component
      if (inComponentsSection) {
        const match = line.match(
          /^\s*-\s*\[([^\]]+)\]\(https:\/\/www\.sy-inc\.com\/docs\/native\/components\/[a-z]/,
        );
...
/**
 * Main function to list all available SY INC Native components.
 */
async function main() {
